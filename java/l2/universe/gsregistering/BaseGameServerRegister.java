 ...
 */
package l2.universe.gsregistering;
...
import l2.universe.i18n.LanguageControl;
import l2.universe.loginserver.GameServerTable;
...
public abstract class BaseGameServerRegister
{
	...
	public static void main(String[] args)
	{
		...
		for (int i = 0 ; i < args.length; i++)
		{
			...
			/*
			 * --fallback If an register operation fails due to ID already being in use it will then
			 * try to register first available ID
			 */
			...
			/*
			 * --register <id> <hexid_dest_dir> Register GameServer with ID <id> and output hexid on
			 * <hexid_dest_dir> Fails if <id> already in use, unless -force is used (overwrites)
			 */
			...
			/*
			 * --language <locale> Sets the app to use the specified locale, overriding
			 * auto-detection
			 */
			else if (arg.equals("-l") || arg.equals("--language"))
			{
				...
				if (locale == null)
				{
					System.out.println("Specified locale '" + loc
					        + "' was not found, using default behaviour.");
				}
				else
				{
					try
