 ...
 */
package l2.universe.gameserver.model.quest;
...
import l2.universe.gameserver.network.serverpackets.ExShowQuestMark;
...
import l2.universe.gameserver.network.serverpackets.TutorialCloseHtml;
import l2.universe.gameserver.network.serverpackets.TutorialEnableClientEvent;
import l2.universe.gameserver.network.serverpackets.TutorialShowHtml;
import l2.universe.gameserver.network.serverpackets.TutorialShowQuestionMark;
...
public final class QuestState
{
	 ...
	 * @link l2.universe.loginserver.serverpacket.QuestList
	 */
	 ...
	 * @param count
	 */
	public void giveReward(int itemId, long amount) 
	{
		rewardItems(itemId, amount);
	}
	...
	public void rewardItems(int itemId, long count)
	{
		...
		else
		{
			...
			else if (_tmpItem.isAccessory())
			{
				count = (long) (count * Config.RATE_QUEST_REWARD_ACCESSORY);
			}
			else if (_tmpItem.isArmorOrShield())
			{
				count = (long) (count * Config.RATE_QUEST_REWARD_ARMOR);
			}
			else if (_tmpItem.isWeapon())
			{
				count = (long) (count * Config.RATE_QUEST_REWARD_WEAPON);
			}
			else
			{
				count = (long) (count * Config.RATE_QUEST_REWARD);
		...
		giveItems(itemId, count, 0, (byte) -1, 0);
	}
	
	/**
	 * Give reward to player using multiplier's. This are special, are for quests where reward is based on 
	 * collection of quest items
	 * @param itemId
	 * @param count
	 */
	public void giveRewardQIBased(int itemId, long count)
	{
		if (count <= 0)
			return;
		...
		if (itemId == 57)
		{
			count = (long) (count * Config.RATE_QUEST_REWARD_ADENA_QI_BASED);
		}
		else
		{
			L2ItemInstance _tmpItem = ItemTable.getInstance().createDummyItem(itemId);
			...
			if(_tmpItem.isEtcItem())
			{
				...
				{
					case POTION:
						count = (long) (count * Config.RATE_QUEST_REWARD_POTION_QI_BASED);
						break;
					case SCROLL:
						count = (long) (count * Config.RATE_QUEST_REWARD_SCROLL_QI_BASED);
						break;
					case RECIPE:
						count = (long) (count * Config.RATE_QUEST_REWARD_RECIPE_QI_BASED);
						break;
					case MATERIAL:
						count = (long) (count * Config.RATE_QUEST_REWARD_MATERIAL_QI_BASED);
						break;
					default:
						count = (long) (count * Config.RATE_QUEST_REWARD_QI_BASED);
						break;
				}
			}
			else if (_tmpItem.isAccessory())
			{
				count = (long) (count * Config.RATE_QUEST_REWARD_ACCESSORY_QI_BASED);
			}
			else if (_tmpItem.isArmorOrShield())
			{
				count = (long) (count * Config.RATE_QUEST_REWARD_ARMOR_QI_BASED);
			}
			else if (_tmpItem.isWeapon())
			{
				count = (long) (count * Config.RATE_QUEST_REWARD_WEAPON_QI_BASED);
			}
			else
			{
				count = (long) (count * Config.RATE_QUEST_REWARD_QI_BASED);
	...
	public void giveItems(int itemId, long amount, int enchantLevel) 
	{
		giveItems(itemId, amount, enchantLevel, (byte)-1, 0);
	}

	public void giveItems(int itemId, long amount, byte attributeId, int attributeLevel) 
	{
		giveItems(itemId, amount, 0, attributeId, attributeLevel);
	}

	public void giveItems(int itemId, long count, int enchantLevel, byte attributeId, int attributeLevel)
	{
		if (count <= 0)
			return;
	...
	/**
	 * Drop Quest item using Config.RATE_QUEST_DROP with 100% chance to drop item.
	 ...
	 */
	public boolean dropQuestItems(int itemId, int count, long neededCount, boolean sound)
	{
		return dropQuestItems(itemId, count, count, neededCount, L2DropData.MAX_CHANCE, sound);
	}
	 ...
	 * @param count(minCount, maxCount) : long Quantity of items to be dropped
	 * @param neededCount : Quantity of items needed for quest
	 * @param dropChance : chance of drop, 100 = 100% chance ...
	 * @param sound : boolean indicating whether to play sound
	 * @return boolean indicating whether player has requested number of items
	 */
	public boolean dropQuestItems(int itemId, int count, long neededCount, float dropChance, boolean sound)
	{
		return dropQuestItems(itemId, count, count, neededCount, dropChance * 10000, sound);
	}
	
	/**
	 * Drop Quest item using Config.RATE_QUEST_DROP
	 * @param itemId : int Item Identifier of the item to be dropped
	 * @param minCount : long min Quantity of items to be dropped
	 * @param maxCount : long max Quantity of items to be dropped
	 * @param neededCount : Quantity of items needed for quest
	 * @param dropChance : chance of drop, 100 = 100% chance ...
	 * @param sound : boolean indicating whether to play sound
	 * @return boolean indicating whether player has requested number of items
	 */
	public boolean dropQuestItems(int itemId, int minCount, int maxCount, long neededCount, float dropChance, boolean sound)
	{		
		return dropQuestItems(itemId, minCount, maxCount, neededCount, false, dropChance, sound);
	}
	...
	{	
		...
		*/
		...
		// TODO: Need to Rework
		// This new system not work fine: on minCount=0, maxCount=1, dropChance=100, Config.RATE_QUEST_DROP=10 => allays drop 10 items
		// 								: on minCount=0, maxCount=1, dropChance= 20, Config.RATE_QUEST_DROP=10 => allays drop 2 items
		// Synerge - New quest drop calculation system
	    int adjDropChance = (int)(dropChance * 10.0F * Config.RATE_QUEST_DROP);
	    int adjMaxCount = (int)(maxCount * Config.RATE_QUEST_DROP);
	    long itemCount = (long)(minCount * Config.RATE_QUEST_DROP) + adjDropChance / 1000;
	    int countDiff = adjMaxCount - minCount;

	    int adjDropChanceRest = adjDropChance % 1000;

	    if (adjDropChanceRest > 0)
	    {
	    	for (int i = 0; i < countDiff; i++)
	    	{
	    		if (Rnd.get(1000) >= adjDropChanceRest)
	    			continue;
		...
		if (itemCount > 0)
		{
			...
			if (sound)
			{
				if (neededCount == 0L)
					...
					playSound((currentCount % neededCount + itemCount < neededCount) ? "Itemsound.quest_itemget" : "Itemsound.quest_middle");
			}
		}
	 ...
	 * @param sp
	 */
	public void addRewardExpAndSp(int exp, int sp) 
	{
		addExpAndSp(exp, sp);
	}
	  
	// Synerge - Quest exp sp rewards are only for players, not shared with pets
	public void addExpAndSp(int exp, int sp)
	{
        exp *= getPlayer().getStat().getRExp();
        sp *= getPlayer().getStat().getRSp();
		getPlayer().addExpAndSp((int) getPlayer().calcStat(Stats.EXPSP_RATE, exp * Config.RATE_QUEST_REWARD_XP, null, null), (int) getPlayer().calcStat(Stats.EXPSP_RATE, sp * Config.RATE_QUEST_REWARD_SP, null, null), false, true);
	}
	
	public void addRewardExpAndSpQIBased(int exp, int sp)
	{
        exp *= getPlayer().getStat().getRExp();
        sp *= getPlayer().getStat().getRSp();
		getPlayer().addExpAndSp((int) getPlayer().calcStat(Stats.EXPSP_RATE, exp * Config.RATE_QUEST_REWARD_XP_QI_BASED, null, null), (int) getPlayer().calcStat(Stats.EXPSP_RATE, sp * Config.RATE_QUEST_REWARD_SP_QI_BASED, null, null), false, true);
	}
	...
	public QuestState exitQuest(boolean repeatable)
	{
		...
		if (repeatable)
		{
			...
			// Synerge - Check Quests --> Instances relations
			checkQuestInstanceRelations();
	...
	/* Synerge
	 * If the quest ends or is canceled, then the instances should kick everyone out or that player
	 * in particular, depending the instance, and lock it to make it unable to enter again until
	 * it ends its time. This is to avoid canceling the quest and then entering into a already used
	 * instance with less mobs and all the quest conds reset, that ends with the player stucked in the
	 * instance and unable to continue. We cant destroy the instance as the player has to wait until its over
	 * or he could make it over and over again without waiting.
	 */
	public void checkQuestInstanceRelations()
	{
		switch (getQuest().getQuestIntId())
		{
			// Pailaka Song of Ice and Fire
			case 128:
				InstanceWorld world = InstanceManager.getInstance().getPlayerWorld(getPlayer());
				if (world != null)
				{
					if (world.templateId == 43)
					{
			...
			case 129:
				world = InstanceManager.getInstance().getPlayerWorld(getPlayer());
				if (world != null)
				{
					if (world.templateId == 44)
					{
						final Instance inst = InstanceManager.getInstance().getInstance(world.instanceId);
						if (inst != null)
							inst.removePlayers();
