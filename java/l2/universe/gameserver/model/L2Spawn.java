...
public class L2Spawn
{
	...
	public L2Spawn(L2NpcTemplate mobTemplate) throws SecurityException, ClassNotFoundException, NoSuchMethodException
	{
		...
		Class<?>[] parameters = {int.class, Class.forName("l2.universe.gameserver.templates.chars.L2NpcTemplate")};
		_constructor = Class.forName("l2.universe.gameserver.model.actor.instance." + implementationName + "Instance").getConstructor(parameters);
	}
    ...
    private L2Npc initializeNpcInstance(L2Npc mob)
    {
		...
		if (Config.MOD_CHAMPION_ENABLE)
		{
			...
			(	mob instanceof L2MonsterInstance
				...
				&& Config.MOD_CHAMPION_FREQUENCY > 0 
				&& mob.getLevel()>=Config.MOD_CHAMP_MIN_LVL 
				&& mob.getLevel()<=Config.MOD_CHAMP_MAX_LVL
				&& (Config.MOD_CHAMPION_ENABLE_IN_INSTANCES || getInstanceId() == 0)
			)
			{
				int random = Rnd.get(100);
				if (random < Config.MOD_CHAMPION_FREQUENCY)
