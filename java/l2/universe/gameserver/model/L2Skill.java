...
public abstract class L2Skill implements IChanceSkillTrigger
{
	...
	{
		TARGET_NONE, TARGET_SELF, TARGET_NOSELF, TARGET_ONE, TARGET_PARTY, TARGET_ALLY, TARGET_CLAN, TARGET_PET, TARGET_SUMMON, TARGET_AREA, TARGET_FRONT_AREA, TARGET_BEHIND_AREA, TARGET_AURA, TARGET_FRONT_AURA, TARGET_BEHIND_AURA, TARGET_CORPSE, TARGET_UNDEAD, TARGET_AREA_UNDEAD, TARGET_CORPSE_ALLY, TARGET_CORPSE_CLAN, TARGET_CORPSE_PLAYER, TARGET_CORPSE_PET, TARGET_AREA_CORPSE_MOB, TARGET_CORPSE_MOB, TARGET_AURA_CORPSE_MOB, TARGET_UNLOCKABLE, TARGET_HOLY, TARGET_FLAGPOLE, TARGET_PARTY_MEMBER, TARGET_PARTY_OTHER, TARGET_PARTY_CLAN, TARGET_ENEMY_SUMMON, TARGET_OWNER_PET, TARGET_GROUND, TARGET_EVENT, TARGET_PARTY_NOTME, TARGET_AREA_SUMMON, TARGET_CLAN_MEMBER, TARGET_COUPLE
	}
	...
	private final L2EffectType[] _negateEffects;
	...
	private final boolean _ignoreEffectResists;
	...
	private final boolean _cannotBeEvaded;
	
	private final boolean _bypassPetrification;
	...
	protected L2Skill(StatsSet set)
	{
		...
		str = set.getString("negateEffects", "");
		
		if (str.equals(""))
			_negateEffects = new L2EffectType[0];
		else
		{
			String[] stats = str.split(" ");
			L2EffectType[] array = new L2EffectType[stats.length];
			
			for (int i = 0; i < stats.length; i++)
			{
				L2EffectType type = null;
				try
				{
					type = Enum.valueOf(L2EffectType.class, stats[i]);
				}
				catch (Exception e)
				{
					throw new IllegalArgumentException("SkillId: " + _id + " Enum value of type " + L2EffectType.class.getName() + " required, but found: " + stats[i]);
				}
				
				array[i] = type;
			}
			_negateEffects = array;
		}
		...
		final String reuseHash = set.getString("sharedReuse", null);
		if (reuseHash != null)
		{
			try
			{
				final String[] valuesSplit = reuseHash.split("-");
		...
		_ignoreEffectResists = set.getBool("ignoreEffectResists", false);
		...
		_cannotBeEvaded = set.getBool("cannotBeEvaded", false);
		
		_bypassPetrification = set.getBool("bypassPetrification", false);
	}
	...
	public final L2EffectType[] getNegateEffects()
	{
	...
	*/
	...
	public final boolean ignoreEffectResists()
	{
		return _ignoreEffectResists;
	}
	...
	public final void setIsOffensive(boolean val)
	{
		_isOffensive = val;
	}
	...
	public final boolean isDamageSkill()
	{
		switch (_skillType)
		{
			case PDAM:
	...
	public final L2Object[] getTargetList(L2Character activeChar, boolean onlyFirst, L2Character target)
	{
		...
		switch (targetType)
		{
					...
					if (player.getClan() != null)
					{
						...
						//synchronized (activeChar.getKnownList().getKnownObjects())
						{
							for (L2PcInstance obj : objs)
							{
								...
								if ((DM._started && !ExternalConfig.DM_ALLOW_INTERFERENCE) && !player.isGM())
								{
			...
			{
				if ((!(target instanceof L2Attackable) || !target.isDead()) && onlyFirst)
				{
					activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.TARGET_IS_INCORRECT));
					return _emptyTargetList;
				}
				...
				//synchronized (activeChar.getKnownList().getKnownObjects())
				{
					...
					for (L2Character obj : objs)
					{
						...
						L2Attackable onetarget = (L2Attackable) obj;
						if ((!onetarget.isDead()) && (skillType == L2SkillType.SWEEP))
							continue;
						
						if (skillType == L2SkillType.SWEEP)
						{
							if (activeChar.getObjectId() != onetarget.getIsSpoiledBy())
							{
								// Send a System Message to the L2PcInstance
								activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.SWEEP_NOT_ALLOWED));
								
								// Send a Server->Client packet ActionFailed to the L2PcInstance
								activeChar.sendPacket(ActionFailed.STATIC_PACKET);
								continue;
							}
							else if (!onetarget.isSpoil())
							{
								// Send a System Message to the L2PcInstance
			...
			case TARGET_AREA_UNDEAD:
			{
				...
				// synchronized (cha.getKnownList().getKnownObjects())
				{
					for (L2Character obj : objs)
					{
						...
						// If target is not dead/fake death and not self
			...
			case TARGET_ENEMY_SUMMON:
			{
				if (target instanceof L2Summon)
				{
					...
					L2PcInstance owner = targetSummon.getOwner();
					if (activeChar instanceof L2PcInstance && activeChar.getPet() != targetSummon && !targetSummon.isDead() && (owner.getPvpFlag() != 0 || owner.getKarma() > 0) || (owner.isInsideZone(L2Character.ZONE_PVP) && ((L2PcInstance) activeChar).isInsideZone(L2Character.ZONE_PVP)) || (owner.isInDuel() && ((L2PcInstance) activeChar).isInDuel() && owner.getDuelId() == ((L2PcInstance) activeChar).getDuelId()) || (owner.getClan() != null && ((L2PcInstance) activeChar).getClan() != null && ((L2PcInstance) activeChar).getClan().isAtWarWith(owner.getClanId()) && owner.getClan().isAtWarWith(((L2PcInstance) activeChar).getClanId())))
			...
			case TARGET_CLAN_MEMBER:
			{
				...
				final L2Npc npc = (L2Npc) activeChar;
				final String npcFactionId = npc.getFactionId();
				if (npcFactionId == null || npcFactionId.isEmpty())
					return new L2Character[] { activeChar };
				
				final Collection<L2Object> objs = activeChar.getKnownList().getKnownObjects().values();
				for (L2Object newTarget : objs)
				{
					if (newTarget instanceof L2Npc && npcFactionId.equals(((L2Npc) newTarget).getFactionId()))
					{
						if (!Util.checkIfInRange(getCastRange(), activeChar, newTarget, true))
							continue;
			...
			case TARGET_EVENT:
			{
				...
				if (playerArena != -1)
				{
					...
					// Aura attack
					Collection<L2PcInstance> knownPlayers = player.getKnownList().getKnownPlayersInRadius(250);
					for (L2PcInstance actor : knownPlayers)
					{
	...
	/*
	 * Check if should be target added to the target list false if target is dead, target same as
	 * caster, target inside peace zone, target in the same party with caster, caster can see target
	 * Additional checks if not in PvP zones (arena, siege): target in not the same clan and
	 * alliance with caster, and usual skill PvP check. 
	 * If TvT event is active - performing additional checks.
	 * If TvT Round event is active - performing additional checks.
	 * additional checks.
	 * 
	...
	public static final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena)
	{
		...
		if (player != null)
		{
			if (targetPlayer != null)
			{
				...
				if ((CTF._started && !ExternalConfig.CTF_ALLOW_INTERFERENCE) && !player.isGM())
				{
				...
				// Check for AOE skills cannot be used on observers and gms
				if (targetPlayer.inObserverMode() || targetPlayer.isGM())
					return false;
		...
		{
			// Source is not playable. Suicide attacks hits everyone
			if (caster instanceof L2Attackable && !skill.isSuicideAttack())
			{
				// target is mob
				if (targetPlayer == null)
				{
	...
	public static final boolean checkForAreaOffensiveSkillsIsDead(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena)
	{
	...
	private boolean checkPartyCheckClan(L2Character activeChar, L2Character target)
	{
	...
	*/
	...
	/**
	 * @return Returns the cancelDebuffRate.
	 */
	...
	public boolean bypassPetrification()
	{
		return _bypassPetrification;
	}
	
	/**
	 * @return Returns if is an Adventurer Buff
	 */
	public boolean isAdventurerBuff()
	{
		return ((getId() >= 4322 && getId() <= 4331) || (getId() >= 5627 && getId() <= 5637));
	}
}
