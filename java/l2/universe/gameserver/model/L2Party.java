...
import l2.universe.gameserver.model.entity.DimensionalRift;
import l2.universe.gameserver.network.SystemMessageId;
import l2.universe.gameserver.network.serverpackets.CreatureSay;
import l2.universe.gameserver.network.serverpackets.ExAskModifyPartyLooting;
import l2.universe.gameserver.network.serverpackets.ExCloseMPCC;
import l2.universe.gameserver.network.serverpackets.ExOpenMPCC;
...
import l2.universe.gameserver.network.serverpackets.ExSetPartyLooting;
import l2.universe.gameserver.network.serverpackets.L2GameServerPacket;
import l2.universe.gameserver.network.serverpackets.PartyMemberPosition;
import l2.universe.gameserver.network.serverpackets.PartySmallWindowAdd;
import l2.universe.gameserver.network.serverpackets.PartySmallWindowAll;
import l2.universe.gameserver.network.serverpackets.PartySmallWindowDelete;
...
public class L2Party
{
	 ...
	 * @see l2.universe.gameserver.model.actor.instance.L2PcInstance#isRequestExpired()
	 */
	...
	public synchronized void removePartyMember(L2PcInstance player, boolean sendMessage)
	{
		if (getPartyMembers().contains(player))
		{
			...
			try
            {
                ...
                Collection<L2Character> knownChars = player.getKnownList().getKnownCharacters();
                for (L2Character character : knownChars)
                {
			...
			for (L2Character member : rewardedMembers)
			{
				...
				if (validMembers.contains(member))
				{
					...
					if (!member.isDead())
                    {
                        ...
                        if (member.isPlayer())
                        {
                        	L2PcInstance memberPC = (L2PcInstance)member;
							if (memberPC.getSkillLevel(467) > 0)
							{
								final L2Skill skill = SkillTable.getInstance().getInfo(467, memberPC.getSkillLevel(467));								
								if (skill.getExpNeeded() <= addexp)
									memberPC.absorbSoul(skill,target);
							}
							
                            memberPC.addExpAndSp(addexp, addsp, useVitalityRate);
                            if (addexp > 0)
                            	memberPC.updateVitalityPoints(vitalityPoints, true, false);
                        }
                        else
