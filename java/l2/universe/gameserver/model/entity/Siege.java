...
import l2.universe.gameserver.instancemanager.SiegeGuardManager;
import l2.universe.gameserver.instancemanager.SiegeManager;
import l2.universe.gameserver.instancemanager.SiegeManager.SiegeSpawn;
...
import l2.universe.gameserver.model.L2SiegeClan.SiegeClanType;
import l2.universe.gameserver.model.actor.L2Npc;
import l2.universe.gameserver.model.actor.instance.L2ControlTowerInstance;
import l2.universe.gameserver.model.actor.instance.L2FlameTowerInstance;
...
public class Siege implements Siegable
{
	...
	public class ScheduleEndSiegeTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					sm = SystemMessage.getSystemMessage(SystemMessageId.CASTLE_SIEGE_S1_SECONDS_LEFT);
	...
	public void announceToPlayer(SystemMessage message, boolean bothSides)
	{
		if (bothSides)
		{
			for (L2SiegeClan siegeClans : getParticipateClans())
			{
	...
	public List<L2PcInstance> getAttackersInZone()
	{
		...
		for (L2SiegeClan siegeclan : getAttackerClans())
		{
			...
			for (L2PcInstance player : clan.getOnlineMembers(0))
			{
				...
				if (player.isInSiege() && checkIfInZone(player.getX(), player.getY(), player.getZ()))
					players.add(player);
			}
		}
		return players;
	...
	public final List<L2SiegeClan> getParticipateClans()
	{
		List<L2SiegeClan> _clans = new FastList<L2SiegeClan>();
		
		for (L2SiegeClan defenderClans : getDefenderClans())
			_clans.add(defenderClans);
		for (L2SiegeClan attackerClans : getAttackerClans())
			_clans.add(attackerClans);
		return _clans;
	}
