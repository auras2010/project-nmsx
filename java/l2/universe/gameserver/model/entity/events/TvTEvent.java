...
public class TvTEvent
{
	...
	public static void init()
	{
		...
		_teams[0] = new TvTEventTeam(ExternalConfig.TVT_EVENT_TEAM_1_NAME, ExternalConfig.TVT_EVENT_TEAM_1_COORDINATES);
		_teams[1] = new TvTEventTeam(ExternalConfig.TVT_EVENT_TEAM_2_NAME, ExternalConfig.TVT_EVENT_TEAM_2_COORDINATES);
	}
	
	/**
	 * Starts the participation of the TvTEvent<br>
	 * 1. Get L2NpcTemplate by ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_ID<br>
	...
	{
		L2NpcTemplate tmpl = NpcTable.getInstance().getTemplate(ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_ID);
		
		if (tmpl == null)
		{
			_log.warning("TvTEventEngine[TvTEvent.startParticipation()]: L2NpcTemplate is a NullPointer -> Invalid npc id in ExternalConfigs?");
			return false;
		}
		...
		try
		{
			...
			_npcSpawn.setLocx(ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[0]);
			_npcSpawn.setLocy(ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[1]);
			_npcSpawn.setLocz(ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[2]);
			_npcSpawn.setAmount(1);
			_npcSpawn.setHeading(ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[3]);
	 ...
	 * 2. Close doors specified in ExternalConfigs<br>
	...
	public static boolean startFight()
	{
		...
		if (_teams[0].getParticipatedPlayerCount() < ExternalConfig.TVT_EVENT_MIN_PLAYERS_IN_TEAMS || _teams[1].getParticipatedPlayerCount() < ExternalConfig.TVT_EVENT_MIN_PLAYERS_IN_TEAMS)
		{
			// Set state INACTIVE
		...
		if (ExternalConfig.TVT_EVENT_IN_INSTANCE)
		{
			try
			{
				_TvTEventInstance = InstanceManager.getInstance().createDynamicInstance(ExternalConfig.TVT_EVENT_INSTANCE_FILE);
				...
				InstanceManager.getInstance().getInstance(_TvTEventInstance).setEmptyDestroyTime(ExternalConfig.TVT_EVENT_START_LEAVE_TELEPORT_DELAY * 1000 + 60000L);
			}
			catch (Exception e)
			{
				_TvTEventInstance = 0;
		...
		// Opens all doors specified in ExternalConfigs for tvt
		openDoors(ExternalConfig.TVT_DOORS_IDS_TO_OPEN);
		// Closes all doors specified in ExternalConfigs for tvt
		closeDoors(ExternalConfig.TVT_DOORS_IDS_TO_CLOSE);
		// Set state STARTED
	...
	public static String calculateRewards()
	{
		if (_teams[0].getPoints() == _teams[1].getPoints())
		{
			...
			if (ExternalConfig.TVT_REWARD_TEAM_TIE)
			{
				rewardTeam(_teams[0]);
				rewardTeam(_teams[1]);
	...
	private static void rewardTeam(TvTEventTeam team)
	{
		...
		for (L2PcInstance playerInstance : team.getParticipatedPlayers().values())
		{
			...
			for (int[] reward : ExternalConfig.TVT_EVENT_REWARDS)
			{
	...
	public static void stopFight()
	{
		...
		unSpawnNpc();
		// Opens all doors specified in ExternalConfigs for tvt
		openDoors(ExternalConfig.TVT_DOORS_IDS_TO_CLOSE);
		// Closes all doors specified in ExternalConfigs for tvt
		closeDoors(ExternalConfig.TVT_DOORS_IDS_TO_OPEN);
		...
		for (TvTEventTeam team : _teams)
		{
			for (L2PcInstance playerInstance : team.getParticipatedPlayers().values())
			{
				...
				if (playerInstance != null)
				{
					new TvTEventTeleporter(playerInstance, ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_COORDINATES, false, false);
				}
			}
		}
	...
	{
		return ExternalConfig.TVT_EVENT_PARTICIPATION_FEE[0] != 0 && ExternalConfig.TVT_EVENT_PARTICIPATION_FEE[1] != 0;
	}
	
	public static boolean hasParticipationFee(L2PcInstance playerInstance)
	{
		return playerInstance.getInventory().getInventoryItemCount(ExternalConfig.TVT_EVENT_PARTICIPATION_FEE[0], -1) >= ExternalConfig.TVT_EVENT_PARTICIPATION_FEE[1];
	}

	public static boolean payParticipationFee(L2PcInstance playerInstance)
	{
		final int itemId = ExternalConfig.TVT_EVENT_PARTICIPATION_FEE[0];
		final int itemNum = ExternalConfig.TVT_EVENT_PARTICIPATION_FEE[1];
	...
	public static void onLogout(L2PcInstance playerInstance)
	{
		if (playerInstance != null && (isStarting() || isStarted() || isParticipating()))
		{
			if (removeParticipant(playerInstance.getObjectId()))
				playerInstance.setXYZInvisible(ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[0] + Rnd.get(101)-50,
						ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[1] + Rnd.get(101)-50,
						ExternalConfig.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[2]);
		}
	}
	...
	public static synchronized void onBypass(String command, L2PcInstance playerInstance, boolean voiceCommand)
	{
		...
		if (command.equals("tvt_event_participation"))
		{
			...
			else if (playerLevel < ExternalConfig.TVT_EVENT_MIN_LVL || playerLevel > ExternalConfig.TVT_EVENT_MAX_LVL)
			{
				htmContent = HtmCache.getInstance().getHtm(playerInstance.getHtmlPrefix(), htmlPath + "Level.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%min%", String.valueOf(ExternalConfig.TVT_EVENT_MIN_LVL));
					npcHtmlMessage.replace("%max%", String.valueOf(ExternalConfig.TVT_EVENT_MAX_LVL));
				}
			}
			else if (_teams[0].getParticipatedPlayerCount() == ExternalConfig.TVT_EVENT_MAX_PLAYERS_IN_TEAMS && _teams[1].getParticipatedPlayerCount() == ExternalConfig.TVT_EVENT_MAX_PLAYERS_IN_TEAMS)
			{
				htmContent = HtmCache.getInstance().getHtm(playerInstance.getHtmlPrefix(), htmlPath + "TeamsFull.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(ExternalConfig.TVT_EVENT_MAX_PLAYERS_IN_TEAMS));
				}
			}
	...
	public static boolean onAction(L2PcInstance playerInstance, int targetedPlayerObjectId)
	{
		...
		if (playerTeamId != -1 && targetedPlayerTeamId != -1 && playerTeamId == targetedPlayerTeamId && playerInstance.getObjectId() != targetedPlayerObjectId && !ExternalConfig.TVT_EVENT_TARGET_TEAM_MEMBERS_ALLOWED)
			return false;
	...
	public static boolean onScrollUse(int playerObjectId)
	{
		...
		if (isPlayerParticipant(playerObjectId) && !ExternalConfig.TVT_EVENT_SCROLL_ALLOWED)
			return false;
	...
	public static boolean onPotionUse(int playerObjectId)
	{
		...
		if (isPlayerParticipant(playerObjectId) && !ExternalConfig.TVT_EVENT_POTIONS_ALLOWED)
			return false;
	...
	public static boolean onItemSummon(int playerObjectId)
	{
		...
		if (isPlayerParticipant(playerObjectId) && !ExternalConfig.TVT_EVENT_SUMMON_BY_ITEM_ALLOWED)
			return false;
	...
	public static void onKill(L2Character killerCharacter, L2PcInstance killedPlayerInstance)
	{
		...
		final L2PcInstance killerPlayerInstance = killerCharacter.getActingPlayer();
		...
		if (killerTeamId != -1 && killedTeamId != -1 && killerTeamId != killedTeamId)
		{
			...
			final CreatureSay cs = new CreatureSay(killerPlayerInstance.getObjectId(), Say2.TELL, killerPlayerInstance.getName(), "I have killed " + killedPlayerInstance.getName() + "!");			
	...
	public static void onTeleported(L2PcInstance playerInstance)
	{
		...
		{
			if (ExternalConfig.TVT_EVENT_MAGE_BUFFS != null && !ExternalConfig.TVT_EVENT_MAGE_BUFFS.isEmpty())
			{
				for (int i : ExternalConfig.TVT_EVENT_MAGE_BUFFS.keys())
				{
					final L2Skill skill = SkillTable.getInstance().getInfo(i, ExternalConfig.TVT_EVENT_MAGE_BUFFS.get(i));
					if (skill != null)
		...
		{
			if (ExternalConfig.TVT_EVENT_FIGHTER_BUFFS != null && !ExternalConfig.TVT_EVENT_FIGHTER_BUFFS.isEmpty())
			{
				for (int i : ExternalConfig.TVT_EVENT_FIGHTER_BUFFS.keys())
				{
					final L2Skill skill = SkillTable.getInstance().getInfo(i, ExternalConfig.TVT_EVENT_FIGHTER_BUFFS.get(i));
					if (skill != null)
						skill.getEffects(playerInstance, playerInstance);
				}
			}
