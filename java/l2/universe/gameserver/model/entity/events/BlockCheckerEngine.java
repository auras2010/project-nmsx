...
import l2.universe.gameserver.model.actor.instance.L2BlockInstance;
...
import l2.universe.gameserver.network.serverpackets.ExCubeGameChangePoints;
import l2.universe.gameserver.network.serverpackets.ExCubeGameCloseUI;
import l2.universe.gameserver.network.serverpackets.ExCubeGameEnd;
import l2.universe.gameserver.network.serverpackets.ExCubeGameExtendedChangePoints;
...
public final class BlockCheckerEngine
{
		...
		public void run()
		{
			...
			switch(_round)
			{
				...
				case 3:
					...
					_task = ThreadPoolManager.getInstance().scheduleGeneral(new CountDown(), 175000);
					break;
			}
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
					{
						...
						public void run()
						{
							if(_girlNpc == null) 
								return;
							_girlNpc.deleteMe();
							SpawnTable.getInstance().deleteSpawn(_girlNpc.getSpawn(), false);
						}
					}, 9000);
				}
				catch(Exception e) 
	...
	class CountDown implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				Thread.sleep(1000);
				_holder.broadCastPacketToTeam(SystemMessage.getSystemMessage(SystemMessageId.BLOCK_CHECKER_ENDS_4));
				Thread.sleep(1000);
				_holder.broadCastPacketToTeam(SystemMessage.getSystemMessage(SystemMessageId.BLOCK_CHECKER_ENDS_3));
				Thread.sleep(1000);
				_holder.broadCastPacketToTeam(SystemMessage.getSystemMessage(SystemMessageId.BLOCK_CHECKER_ENDS_2));
				Thread.sleep(1000);
				_holder.broadCastPacketToTeam(SystemMessage.getSystemMessage(SystemMessageId.BLOCK_CHECKER_ENDS_1));
	...
	{		
		...
		private void rewardAsLooser(boolean isRed)
		{
			...
			for(L2PcInstance player : tempPoints.keySet())
			{
