...
public class TvTManager
{
	...
	private TvTManager()
	{
		if (ExternalConfig.TVT_EVENT_ENABLED)
		{
			TvTEvent.init();
			
			this.scheduleEventStart();
			_log.info("[Team VS Team] EventEngine: Engine started.");
		}
		else
		{
			_log.info("[Team VS Team] EventEngine: Engine is disabled.");
		}
	}
	...
	public void scheduleEventStart()
	{
		try
		{
			...
			for (String timeOfDay : ExternalConfig.TVT_EVENT_INTERVAL)
			{
		...
		{
			_log.warning("TvTEventEngine[TvTManager.scheduleEventStart()]: Error figuring out a start time. Check TvTEventInterval in ExternalConfig file.");
		}
	}
	...
	public void startReg()
	{
		...
		else
		{
			...
			_task.setStartTime(System.currentTimeMillis() + 60000L * ExternalConfig.TVT_EVENT_PARTICIPATION_TIME);
			ThreadPoolManager.getInstance().executeTask(_task);
		}
	}
	...
	public void startEvent()
	{
		...
		{
			TvTEvent.sysMsgToAllParticipants("TvT Event: Teleporting participants to an arena in " + ExternalConfig.TVT_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
			_task.setStartTime(System.currentTimeMillis() + 60000L * ExternalConfig.TVT_EVENT_RUNNING_TIME);
			ThreadPoolManager.getInstance().executeTask(_task);
		}
	}
	...
	public void endEvent()
	{
		...
		TvTEvent.sysMsgToAllParticipants("TvT Event: Teleporting back to the registration npc in " + ExternalConfig.TVT_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
		TvTEvent.stopFight();
