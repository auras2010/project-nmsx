...
import l2.universe.gameserver.FortUpdater;
import l2.universe.gameserver.FortUpdater.UpdaterType;
...
public class Fort
{
	...
	public boolean setOwner(L2Clan clan, boolean updateClansReputation)
	{
		...
		else
		{
			...
			spawnSpecialEnvoys();
			// Prepare 1hr task for special envoys despawn
	...
	private void loadDoor()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				L2DoorInstance door = DoorTable.parseList(_doorDefault.get(_doorDefault.size() - 1), true);
				_doors.add(door);
