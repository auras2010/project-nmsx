...
public class ZoneNPoly extends L2ZoneForm
{
	...
	public Point2D intersectsSegment(int fromX, int fromY, int toX, int toY)
	{
		...
		// Check every possible line of the polygon for an intersection with given segment
		for (int i = 0; i < _y.length; i++)
		{
			tX = _x[i];
			tY = _y[i];
			...
			// Check if this line intersects any of the sites of the poly
			if (lineSegmentsIntersect(tX, tY, uX, uY, fromX, fromY, toX, toY))
			{
				return getIntersectionPoint(new Line2D.Double(tX, tY, uX, uY), new Line2D.Double(fromX, fromY, toX, toY));
			}
		}
		return null; 
	}
