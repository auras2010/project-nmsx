...
import l2.universe.gameserver.model.actor.L2Attackable.AggroInfo;
import l2.universe.gameserver.model.actor.instance.L2DoorInstance;
import l2.universe.gameserver.model.actor.instance.L2MerchantSummonInstance;
...
import l2.universe.gameserver.model.actor.knownlist.SummonKnownList;
import l2.universe.gameserver.model.actor.stat.SummonStat;
import l2.universe.gameserver.model.actor.status.SummonStatus;
...
import l2.universe.gameserver.network.serverpackets.ExPartyPetWindowAdd;
import l2.universe.gameserver.network.serverpackets.ExPartyPetWindowDelete;
import l2.universe.gameserver.network.serverpackets.ExPartyPetWindowUpdate;
import l2.universe.gameserver.network.serverpackets.L2GameServerPacket;
import l2.universe.gameserver.network.serverpackets.PetDelete;
import l2.universe.gameserver.network.serverpackets.PetInfo;
import l2.universe.gameserver.network.serverpackets.PetItemList;
import l2.universe.gameserver.network.serverpackets.PetStatusUpdate;
...
public abstract class L2Summon extends L2Playable
{
    ...
    private Future<?> _checkStuckTask = null;
    ...
    public void onSpawn()
    {
        ...
        /* Synerge - Start task to check if the summon gets stuck somewhere, then teleport it to his owner, each 30 seconds
         * Dont teleport Siege Summons
         */
        /**
        if (!(this instanceof L2SiegeSummonInstance))
        	_checkStuckTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new CheckStuckTask(), 60000, 30000);
        	*/
    }
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (owner != null)
		{
			...
			for (L2Character TgMob : KnownTarget)
			{
				...
				L2Attackable attMob = (L2Attackable) TgMob;
				if (attMob.isDead())
					continue;
				
				AggroInfo info = attMob.getAggroList().get(this);
				if (info != null)
					attMob.addDamageHate(owner, info.getDamage(), info.getHate());
			}
		}
		
		// Synerge - Stop antistuck thread
		if (_checkStuckTask != null)
		{
			_checkStuckTask.cancel(false);
			_checkStuckTask = null;
		}
	...
	/* 
	 * Synerge - New system to avoid loosing pets and summons in walls or dissappearing.
	 * The idea is make a thread that each 30 seconds checks if the summons is far away from the owner and is not doing anything
	 * in that case, teleport him to the owner
	 */
	@SuppressWarnings("unused")
	private final class CheckStuckTask implements Runnable
	{
		public void run()
		{
			try
			{
				if (getOwner() == null || L2Summon.this == null || (isDead() && L2Summon.this instanceof L2SummonInstance))
				{
		    		// Synerge - Stop antistuck thread
		    		if (_checkStuckTask != null)
		    		{
				...
				/* If the summon is doing something, avoid the check */
				if (getOwner().isTeleporting() || isTeleporting() || isCastingNow() || isAttackingNow() || getAI().isAutoAttacking() || getOwner().getAI().isAutoAttacking())
					return;
				...
				if (!Util.checkIfInRange(2000, L2Summon.this, getOwner(), true)) 
				{
					teleToLocation(getOwner().getX(), getOwner().getY(), getOwner().getZ(), false);
					setFollowStatus(true);
				}
			}
			catch (Exception e)
			{
				_log.log(Level.WARNING, "Error in AntiStucking Summon thread", e);
			}
		}
	}
}
