...
import l2.universe.gameserver.model.L2TeleportLocation;
...
import l2.universe.gameserver.network.serverpackets.ExShowDominionRegistry;
...
public class L2CastleChamberlainInstance extends L2MerchantInstance
{
	...
	private static final int LORDS_CROWN_ID = 6841;
	private int _preDay;
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (condition == COND_OWNER)
		{
			...
			else if (actualCommand.equals("give_crown"))
			{
				...
				if (player.isClanLeader() && player.getClan().getHasCastle() > 0)
				{
					if (player.getInventory().getItemByItemId(LORDS_CROWN_ID) == null)
					{
						L2ItemInstance crown = player.getInventory().addItem("Crown", LORDS_CROWN_ID, 1, player, this);
						if (crown != null)
							player.sendPacket(new InventoryUpdate());

						SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.EARNED_ITEM_S1);
						sm.addItemName(crown);
