...
import l2.universe.gameserver.model.actor.status.SiegeFlagStatus;
...
public class L2SiegeFlagInstance extends L2Npc
{
	...
	public L2SiegeFlagInstance(L2PcInstance player, int objectId, L2NpcTemplate template, boolean advanced, boolean outPost)
	{
		...
		final int pX = _player.getX();
		final int pY = _player.getY();
		final int pZ = _player.getZ();
		
		SiegableHall hall = CHSiegeManager.getInstance().getNearbyClanHall(pX, pY, 10000);
		if(_clan != null && hall != null && hall.isInSiege())
		{
			if(hall.getAttackers().contains(_clan))
				hall.addSiegeFlag(_clan);
		}
		else
		{
			_siege = SiegeManager.getInstance().getSiege(pX, pY, pZ);
			if (_siege == null)
