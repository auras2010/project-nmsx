...
public class CharStat
{
	...
	public int getEvasionRate(L2Character target)
	{
		...
		/**
		if (val > Config.MAX_EVASION && (!_activeChar.isGM() || !Config.NO_STATS_LIMITS_FOR_GMS))
	...
	public int getMAtk(L2Character target, L2Skill skill)
	{
    	...
    	float bonusAtk = 1;
    	if (Config.MOD_CHAMPION_ENABLE && _activeChar.isChampion())
    		bonusAtk = Config.MOD_CHAMPION_ATK;
    	else if (_activeChar.isRaid())
    		bonusAtk *= Config.RAID_MATTACK_MULTIPLIER;
    	else if (_activeChar instanceof L2MonsterInstance)
    		bonusAtk *= Config.MOB_MATTACK_MULTIPLIER;
	...
	public int getMAtkSpd()
	{
    	...
    	float bonusSpdAtk = 1;
    	if (Config.MOD_CHAMPION_ENABLE && _activeChar.isChampion())
    		bonusSpdAtk = Config.MOD_CHAMPION_SPD_ATK;
    	
		double val = calcStat(Stats.MAGIC_ATTACK_SPEED, _activeChar.getTemplate().baseMAtkSpd * bonusSpdAtk, null, null);
		/**
		if (val > Config.MAX_MATK_SPEED && (!_activeChar.isGM() || !Config.NO_STATS_LIMITS_FOR_GMS))
	...
	public float getMovementSpeedMultiplier()
	{
		...
		float base = getActiveChar().getTemplate().baseRunSpd;
		if (base == 0)
			return 1;
	...
	public int getPAtk(L2Character target)
	{
		...
		if  (Config.MOD_CHAMPION_ENABLE && _activeChar.isChampion())
    		...
    		bonusAtk *= Config.MOB_PATTACK_MULTIPLIER;
	...
	public int getPAtkSpd()
	{
    	...
    	float bonusAtk = 1;
        if  (Config.MOD_CHAMPION_ENABLE && _activeChar.isChampion())
    		bonusAtk = Config.MOD_CHAMPION_SPD_ATK;
        
		return (int) Math.round(calcStat(Stats.POWER_ATTACK_SPEED, _activeChar.getTemplate().basePAtkSpd * bonusAtk, null, null));
	}
   ...
   public final double getPDefGiants(L2Character target)
   {
	...
	/** Return the RUNE_OF_EXP rate (base+modifier) of the L2Character. */
	public final double getRExp()
	{
		double val =  calcStat(Stats.RUNE_OF_EXP, 1, null, null);
		if (val > 1.5)
			val = 1.5;
	...
	/** Return the RUNE_OF_SP rate (base+modifier) of the L2Character. */
	public final double getRSp()
	{
		double val =  calcStat(Stats.RUNE_OF_SP, 1, null, null);
		if (val > 1.5)
			val = 1.5;
