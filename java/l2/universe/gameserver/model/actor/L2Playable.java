...
import l2.universe.gameserver.model.actor.knownlist.PlayableKnownList;
import l2.universe.gameserver.model.actor.stat.PlayableStat;
import l2.universe.gameserver.model.actor.status.PlayableStatus;
...
public abstract class L2Playable extends L2Character
{
	...
	/**
	 * Synerge - This is for solving the issue when 2 silent move effects are on, and one wear off, then
	 * the other will also, even if its active.
	 * The only solution that i came with, is to save the number of active silent effects, so when
	 * it reachs to 0 remove it
	 */
	private int _nSilentEffects = 0;
	
	public void setNSilentEffects(int newAmount)
	{
		if (newAmount < 0)
			newAmount = 0;
		
		_nSilentEffects = newAmount;
	}
	
	public int getNSilentEffects()
	{
		return _nSilentEffects;
	}
	
	/** Synerge - Support for Betrayal Mark effect */	
	private boolean _isUnderBetrayalMark = false; // if character is under Betrayal Mark.
	...
	/** Synerge - Support for transfering damage to a party member */
	private boolean _isTransferringDmg = false; // if character is transferring damage to L2PcInstance character. (Shield of Faith skill)
	private L2Playable _isTransferringDmgTo = null; // L2PcInstance character where damage is transferred. (Shield of Faith skill)
