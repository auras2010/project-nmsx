...
import l2.universe.gameserver.model.SpawnListener;
	...
	*/
	...
	public boolean setFinalScore(L2PcInstance player, int oracle, int festivalId, long offeringScore)
	{
		...
		if (offeringScore > thisCabalHighScore)
		{
			...
			/* Synerge - This cant go, this says Dusk wins a festival first, the
			 * dawn do it, so dusk have x points, dawn 0 points, 0 < x always
			 * so dawn will never register points oO. Same backwards, this is the problem
			 * that blocks registering points in the festival
			 */
			/**
			...
			/* Synerge - Another error. If dawn win a festival then 150 points to dawn, 0 to dusk
			 * If dawn wins again the same festival with more points, then dawn will have 300 points, dusk -150 limit to 0
			 * Only add or remove points if the other is winning the festival, otherwise is just a new score
			 */
			if (offeringScore > otherCabalHighScore && thisCabalHighScore <= otherCabalHighScore)
			{
