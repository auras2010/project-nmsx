...
public class MailManager
{
	...
	public final void sendSystemMail(L2PcInstance sender, String subject, String text)
	{
		Connection con = null;
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT charId FROM characters WHERE accesslevel > -1 AND deletetime = 0");
			ResultSet rset = statement.executeQuery();
			while (rset.next())
			{
				final int charId = rset.getInt("charId");
				if (charId == sender.getObjectId())
					continue;
				
				Message sysMail = new Message(sender.getObjectId(), charId, false, subject, text, 0);
				sysMail.setDeletedBySender();
				sysMail.setIsNews(true);
				sendMessage(sysMail);
			}
			sender.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.MAIL_SUCCESSFULLY_SENT));
		...
		{
			_log.log(Level.SEVERE, "MailManager: Failed sending system mail by " + sender.getName() +" : " + e);
		}
		finally
		{
			L2DatabaseFactory.close(con);
