...
import l2.universe.gameserver.model.L2MinionData;
...
public class NpcTable
{
	...
	private void restoreNpcData()
	{
		...
		try
		{
			...
			if (Config.CUSTOM_MINIONS_TABLE)
			{
				try
				{
					PreparedStatement statementCM = con.prepareStatement("SELECT " + L2DatabaseFactory.getInstance().safetyString(new String[] { "boss_id", "minion_id", "amount_min", "amount_max" }) + " FROM custom_minions");
					ResultSet minionData = statementCM.executeQuery();
					L2MinionData minionDat = null;
					L2NpcTemplate npcDat = null;
					int cnt = 0;
					...
					while (minionData.next())
					{
						...
						if (npcDat == null)
						{
							_log.warning("Custom Minion references undefined boss NPC. Boss NpcId: " + raidId);
							continue;
						}
					...
					minionData.close();
					statementCM.close();
					_log.info("CMinionTable: Loaded " + cnt + " Custom Minions.");
				}
				catch (Exception e)
				{
					_log.log(Level.SEVERE, "CMinionTable: Error loading custom minion data.", e);
				}
			}
			...
			try 
			{
				...
				{
					final int npc_id = NpcAIDataTable.getInt("npc_id");
	...
	private void fillNpcTable(ResultSet NpcData, boolean customData) throws Exception
	{
		...
		while (NpcData.next())
		{
			...
			final int idTemp = NpcData.getInt("idTemplate");
