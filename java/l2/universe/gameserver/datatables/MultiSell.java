...
import l2.universe.gameserver.network.serverpackets.MultiSellList;
	...
	public static final boolean checkSpecialIngredient(int id, long amount, L2PcInstance player)
	{
		switch (id)
		{
			...
			case PC_BANG_POINTS:
				...
				{
					player.sendPacket(SystemMessage.sendString("You need more PC Bang Points"));
					break;
				}
				return true;
			case FAME:
	...
	public static final void addSpecialProduct(int id, long amount, L2PcInstance player)
	{
		switch (id)
		{
			case CLAN_REPUTATION:
				...
				break;
			case PC_BANG_POINTS:
				player.addPcBangScore((int)amount);
				player.sendPacket(new UserInfo(player));
				player.sendPacket(new ExBrExtraUserInfo(player));
				break;
			case FAME:
