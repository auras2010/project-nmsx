...
import l2.universe.gameserver.LoginServerThread.SessionKey;
...
import l2.universe.util.EventData;
			...
			try
			{
				...
				if (player != null && !isDetached())
				{
							...
							&& player.getVehicle() == null)
					{
						...
						{
							if (MultiBoxProtection.getInstance().checkMultiBoxOfflineStore(L2GameClient.this, MultiBoxProtection.getInstance().getLocalIp(player.getAccountName())))
							{
								player.leaveParty();
								if (Config.OFFLINE_SET_NAME_COLOR)
								{
									player.getAppearance().setVisibleNameColor(Config.OFFLINE_NAME_COLOR);
