...
import static l2.universe.gameserver.model.actor.L2Character.ZONE_PEACE;
...
import l2.universe.gameserver.model.entity.Message;
import l2.universe.gameserver.model.itemcontainer.Mail;
import l2.universe.gameserver.network.SystemMessageId;
import l2.universe.gameserver.network.serverpackets.ExNoticePostSent;
	...
	private final boolean removeItems(L2PcInstance player, Message msg)
	{
		...
		if (_items != null)
		{
			for (AttachmentItem i : _items)
			{
				...
				if (item == null || !(item.isTradeable() || (!item.isTradeable() && player.isGM() && Config.GM_TRADE_RESTRICTED_ITEMS)) || item.isEquipped())
				{
		...
		for (AttachmentItem i : _items)
		{
			...
			if (oldItem == null || !(oldItem.isTradeable() || (!oldItem.isTradeable() && player.isGM() && Config.GM_TRADE_RESTRICTED_ITEMS)) || oldItem.isEquipped())
			{
