	...
	public static final class EnchantScroll extends EnchantItem
	{
		...
		public final int getChance(L2ItemInstance enchantItem, EnchantItem supportItem)
		{
			...
			if (_isBlessed)
			{
				...
				if (_isWeapon)
				{
					if (enchantItem.getWeaponItem().isMagicWeapon())
						chance = Config.BLESSED_ENCHANT_CHANCE_WEAPON_MAGE;
					else
						chance = Config.BLESSED_ENCHANT_CHANCE_WEAPON_WARRIOR;
				}
				else if (isAccessory)
			...
			else
			{
				...
				{
					if (enchantItem.getWeaponItem().isMagicWeapon())
						chance = Config.ENCHANT_CHANCE_WEAPON_MAGE;
					else
						chance = Config.ENCHANT_CHANCE_WEAPON_WARRIOR;
				}
				else if (isAccessory)
