...
import l2.universe.gameserver.network.serverpackets.ShowCalculator;
...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		// 	{
			 ...
			 */
			if (activeChar.isSitting() || (!Config.ALT_GAME_KARMA_PLAYER_CAN_TELEPORT && activeChar.getKarma() > 0))
			{
				switch (_itemId)
				{
					case 736:
					...
					case 12753:
						return;
					default:
						if (_itemId >= 7117 && _itemId <= 7135 || _itemId >= 13395 && _itemId <= 13414)
							return;
						break;
				}
			}
			...
			if (activeChar.isStunned()
				...
				|| activeChar.isAfraid()
				|| (activeChar.isCastingNow() && !(item.isPotion() 
				|| item.isElixir() && !item.canBeUsedDuringCombat())))
			{
				return;
			}
			...
			// Synerge - If the script returns false, return
			if (!ScriptHandler.getInstance().execute(CallSite.ON_USE_ITEM, activeChar, item))
				return;
			...
			if (item.isEquipable())
			{
                ...
                switch (item.getItem().getBodyPart())
                {
                    ...
                    case L2Item.SLOT_R_HAND:
                    {
                        ...
                        // Synerge - If the player is in a mouting transformation, then it cannot equip weapon or shield
                        if (activeChar.getTransformation() != null && activeChar.getTransformation().isMountingTransformation())
                        {
                        	activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CANNOT_EQUIP_ITEM_DUE_TO_BAD_CONDITION));
                        	return;
                        }
