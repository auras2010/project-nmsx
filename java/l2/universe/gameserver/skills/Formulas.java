...
import l2.universe.gameserver.model.L2SiegeClan;
...
import l2.universe.gameserver.model.zone.type.L2CastleZone;
import l2.universe.gameserver.model.zone.type.L2ClanHallZone;
import l2.universe.gameserver.model.zone.type.L2FortZone;
import l2.universe.gameserver.model.zone.type.L2MotherTreeZone;
...
import l2.universe.gameserver.util.StringUtil;
...
public final class Formulas
{
	...
	static class FuncMDefMod extends Func
	{
		...
		public void calc(Env env)
		{
			if (env.player instanceof L2PcInstance)
			{
				...
				PcInventory inv = p.getInventory();
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_LFINGER) != null)
					env.value -= 5;
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_RFINGER) != null)
					env.value -= 5;
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_LEAR) != null)
					env.value -= 9;
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_REAR) != null)
					env.value -= 9;
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_NECK) != null)
					env.value -= 13;
			}
	...
	static class FuncPDefMod extends Func
	{
		...
		public void calc(Env env)
		{
			if (env.player instanceof L2PcInstance)
			{
				...
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_HEAD) != null)
					env.value -= 12;
				L2ItemInstance chest = inv.getPaperdollItem(Inventory.PAPERDOLL_CHEST);
				if (chest != null)
					env.value -= hasMagePDef ? 15 : 31;
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_LEGS) != null || (chest != null && chest.getItem().getBodyPart() == L2Item.SLOT_FULL_ARMOR))
					env.value -= hasMagePDef ? 8 : 18;
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_GLOVES) != null)
					env.value -= 8;
				if (inv.getPaperdollItem(Inventory.PAPERDOLL_FEET) != null)
					env.value -= 7;
			}
			env.value *= env.player.getLevelMod();
		}
	...
	public static final double calcHpRegen(L2Character cha)
	{
		...
		if (Config.MOD_CHAMPION_ENABLE && cha.isChampion())
			hpRegenMultiplier *= Config.MOD_CHAMPION_HP_REGEN;
		...
		else if (cha instanceof L2PcInstance)
		{
			...
			if (player.isInsideZone(L2Character.ZONE_CLANHALL) && clan != null && clan.getHasHideout() > 0)
			{
			...
			if (player.isInsideZone(L2Character.ZONE_CASTLE) && clan != null && clan.getHasCastle() > 0)
			{
	...
	public static final double calcPhysDam(L2Character attacker, L2Character target, L2Skill skill,	byte shld, boolean crit, boolean dual, boolean ss)
	{
		...
		*/
		...
		// Synerge - Damage bonus depending in the position of the character with the target
		damage *= calcAngleMultiplier(attacker, target);
	...
	//--------------------------------------------------------------------
	// Calculate the Blow skill before skill cast, so the animation should be fixed by this	
	public final boolean calcBlowFirst(L2Character activeChar,L2Character target)
	{
		...
		return calcBlow(activeChar, target, _successChance);
	}
	//--------------------------------------------------------------------
    ...
    // Synerge - Physical bonus depending on character position
	/**
	 * Calculates melee damage addition depending on angle between attacker and target
	 * this is how I made it atm, yes it's an inaccurate graph but it makes the point clear :)
	 * 
	 * center of the graph is the target, and the diagonals represent the limit between front, 
	 * side and rear from it, on a 45 angle base, so we basically have:
	 * 0-45: front
	 * 46-135: side (right)
	 * 136-225: rear
	 * 226-315: side (left)
	 ...
	 *           rear
	 *           
	 * calculations are based on headings of both attacker and target, to know which way they are facing
	 * 
	 */
	private static final byte MELEE_SIDE_DMG_BONUS = 5;
	private static final byte MELEE_REAR_DMG_BONUS = 20;
	
	private static double calcAngleMultiplier(L2Character attacker, L2Character target)
	{
		// Angle     =                 Target Heading                   - Attacker position from target(in angles) 
		double angle = Util.convertHeadingToDegree(target.getHeading()) - Util.calculateAngleFrom(attacker, target);
		if (angle < 0)
			angle += 360;
		
		if (angle <= 45 || angle > 315) // Front
			return 1;
		else if (angle > 135 && angle <= 225) // Rear
			return 1 + MELEE_REAR_DMG_BONUS / 100;
		else
			// Sides
			return 1 + MELEE_SIDE_DMG_BONUS / 100;
	}
}
