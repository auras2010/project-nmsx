...
import l2.universe.gameserver.model.AutoSpawnHandler.AutoSpawnInstance;
...
public class SevenSigns
{
	...
	{
		return blueCount * BLUE_CONTRIB_POINTS + greenCount * GREEN_CONTRIB_POINTS + redCount
		        * RED_CONTRIB_POINTS;
	}
	
	public static long calcAncientAdenaReward(long blueCount, long greenCount, long redCount)
	{
		return blueCount * SEAL_STONE_BLUE_VALUE + greenCount * SEAL_STONE_GREEN_VALUE + redCount
		        * SEAL_STONE_RED_VALUE;
	}
	...
	public int getPlayerStoneContrib(int objectId)
	{
		...
		return currPlayer.getInteger("red_stones") + currPlayer.getInteger("green_stones")
		        + currPlayer.getInteger("blue_stones");
	}
	...
	protected void teleLosingCabalFromDungeons(String compWinner)
	{
		...
		//synchronized (L2World.getInstance().getAllPlayers())
		{
			...
			for (L2PcInstance onlinePlayer : pls)
			{
				...
				String playerCabal = currPlayer == null ? "" : currPlayer.getString("cabal");

				if (isSealValidationPeriod() || isCompResultsPeriod())
				{
					if (!onlinePlayer.isGM() && onlinePlayer.isIn7sDungeon() && !playerCabal.equals(compWinner))
					{
				...
				{
					if (!onlinePlayer.isGM() && onlinePlayer.isIn7sDungeon() && playerCabal.isEmpty())
					{
						onlinePlayer.teleToLocation(MapRegionTable.TeleportWhereType.Town);
						onlinePlayer.setIsIn7sDungeon(false);
