...
public class AdminBuffs implements IAdminCommandHandler
{
	...
	{		
		...
		if (command.startsWith("admin_getbuffs"))
		{
			...
			else if (activeChar.getTarget() instanceof L2Character)
			{
		...
		else if (command.startsWith("admin_areacancel"))
		{
			try
			{
				...
				final Collection<L2Character> knows = activeChar.getKnownList().getKnownCharactersInRadius(radius);
				for (final L2Character knownChar : knows)
				{
