...
public class ManaHeal implements ISkillHandler
{
	...
	public void useSkill(final L2Character actChar, final L2Skill skill, final L2Object[] targets)
	{
		for (final L2Character target : (L2Character[]) targets)
		{
			...
			/* Synerge - Retail recharge modifier considering lvl difference
			 * After 6 lvls of difference, 10% of recharge is lost for each lvl, after 15 lvls 0 is recharged
			 */
			if (!skill.isPotion() && skill.getHpConsume() <= 0 && actChar instanceof L2PcInstance && target instanceof L2PcInstance)
			{
				final int magicLvl = (skill.getMagicLevel() > 0 ? skill.getMagicLevel() : actChar.getLevel());
	            final int lvlDiff = target.getLevel() - magicLvl;	            
	            if (lvlDiff > 5)
	            {
	            	mp *= (1 - (lvlDiff - 5) * 0.1);	            	
	            	if (mp < 0)
			...
			{
				final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.S2_MP_RESTORED_BY_C1);
