...
import l2.universe.ExternalConfig; // Add by CTF Event
...
import l2.universe.gameserver.model.L2SummonItem;
...
import l2.universe.gameserver.model.actor.instance.L2XmassTreeInstance;
...
import l2.universe.gameserver.network.serverpackets.ActionFailed; // Add by CTF Event
...
public class SummonItems implements IItemHandler
{
	public void useItem(L2Playable playable, L2ItemInstance item, boolean forceUse)
	{
		...
		// Add by CTF Event
		if (activeChar._inEventCTF && CTF._started && !ExternalConfig.CTF_ALLOW_SUMMON)
		{
		...
		if (activeChar._inEventDM && DM._started && !ExternalConfig.DM_ALLOW_SUMMON)
		{
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
	...
	static class PetSummonFinalizer implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				// Synerge - Checks if this pet can be summoned on this instance. If not, then unsummon. Cant be done earlier
				if (petSummon.getInstanceId() != 0 && ExternalConfig.LIMIT_SUMMONS_PAILAKA)
				{
					if (!_item.canSummonPetOnInstance(petSummon.getInstanceId(), petSummon.getLevel()))
						petSummon.unSummon(petSummon.getOwner());
				}
			}
			catch (final Exception e)
			{
