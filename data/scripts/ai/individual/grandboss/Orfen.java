...
{	
	...
	private static final int MINION_SPAWN_INTERVAL = 360000;
	...
	public void spawnBoss(L2GrandBossInstance npc)
	{
		...
		// Spawn 4 minions guards
		final int x = npc.getX();
		final int y = npc.getY();
		L2Attackable mob = (L2Attackable)addSpawn(RAIKEL_LEOS, x + 100, y + 100, npc.getZ(), 0, false, 0);
	...
	public String onFactionCall(L2Npc npc, L2Npc caller, L2PcInstance attacker, boolean isPet)
	{
		...
		switch (npc.getNpcId())
		{
			...
			case RIBA_IREN:
				...
				if (callerId == ORFEN)
					chance = 9;
				if (callerId != RIBA_IREN && !npc.isCastingNow() && caller.getCurrentHp() < (caller.getMaxHp() / 2.0) && Rnd.get(10) < chance)
				{
					npc.getAI().setIntention(CtrlIntention.AI_INTENTION_IDLE, null, null);
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		switch (npc.getNpcId())
		{
			case ORFEN:
				...
				final long respawnTime = (long) Config.Interval_Of_Orfen_Spawn + Rnd.get(Config.Random_Of_Orfen_Spawn);
				...
				break;
			case RAIKEL_LEOS:
				if (GrandBossManager.getInstance().getBossStatus(ORFEN) == ALIVE)
				{
					_Minions.remove(npc);
					startQuestTimer("spawn_minion", MINION_SPAWN_INTERVAL, npc, null);
				}
				break;
		}
