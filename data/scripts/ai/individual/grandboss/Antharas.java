...
public class Antharas extends L2AttackableAIScript
{
	...
	{
		super(id, name, descr);
		final int[] mobs = { ANTHARASOLDID, ANTHARASWEAKID, ANTHARASNORMALID, ANTHARASSTRONGID, 29069, 29070, 29071, 29072, 29073, 29074, 29075, 29076 };
			...
			if (antharasId != 0)
			{
				switch (status)
				{
					case FIGHTING:
						...
						break;
					case DEAD:
						info = GrandBossManager.getInstance().getStatsSet(antharasId);
						final Long respawnTime = info.getLong("respawn_time");
						if (respawnTime <= System.currentTimeMillis())
						{
							// the time has already expired while the server was offline. Immediately spawn antharas in his cave.
	...
	private class SelfDestructionOfBomber implements Runnable
	{
		...
		public void run()
		{
			...
			switch (_bomber.getNpcId())
			{
				...
				case 29074:
				case 29075:
					_bomber.doCast(SkillTable.getInstance().getInfo(5097, 1));
					break;
				case 29076:
					_bomber.doCast(SkillTable.getInstance().getInfo(5094, 1));
					break;
			}
	...
	private class UnlockAntharas implements Runnable
	{
		...
		public void run()
		{
			...
			{
				final Collection<L2PcInstance> world = L2World.getInstance().getAllPlayers().values();
				for (final L2PcInstance player : world)
				{
					player.sendPacket(new Earthquake(185708, 114298, -8221, 20, 10));
				}
			}
		}
	}
	...
	{
		// If the attacker is not inside the zone, kill it
		if (_Zone != null && !_Zone.isInsideZone(attacker))
		{
		...
		switch (npc.getNpcId())
		{
			...
			case 29067:
			case 29068:
				if (_antharas == null || (_antharas != null && _antharas.getObjectId() != npc.getObjectId()))
					return "";
				_LastAction = System.currentTimeMillis();
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		switch (npc.getNpcId())
		{
			...
			case 29068:
				...
				final long respawnTime = (long) Config.Interval_Of_Antharas_Spawn + Rnd.get(Config.Random_Of_Antharas_Spawn);
