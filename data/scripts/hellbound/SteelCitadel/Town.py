print "Instance: Hellbound - Town"

import sys
from java.lang                                import System
from l2.universe.gameserver.ai                 import CtrlIntention
from l2.universe.gameserver.datatables         import DoorTable
from l2.universe.gameserver.datatables         import SpawnTable
from l2.universe.gameserver.instancemanager    import HellboundManager
from l2.universe.gameserver.instancemanager    import InstanceManager
from l2.universe.gameserver.model.actor        import L2Summon
from l2.universe.gameserver.model.entity       import Instance
...
from l2.universe.gameserver.network                 import SystemMessageId
from l2.universe.gameserver.network.serverpackets   import CreatureSay
...
class HellboundTown(JQuest):
	...
	def onSpawn(self,npc):
	    ...
	    if npcId == AMASKARI:
		    ...
		    for i in range(9):
			    ...
			    newSlave = self.addSpawn(22450,offsetX,offsetY,zz,0,False,0,False,npc.getInstanceId(),0)
			    newSlave.setRunning()
		...
		if npcId == KANAF:
			...
			if instanceId == 0:
			    if not checkNewInstanceConditions(player):
				    return
			    instanceId = InstanceManager.getInstance().createDynamicInstance("HellboundTown.xml")
			    if not self.worlds.has_key(instanceId):
				    ...
				    newKeymaster = self.addSpawn(KEYMASTER,KLOC[0],KLOC[1],KLOC[2],KLOC[3],False,0,False,world.instanceId,0)
	...
	def onKill(self,npc,player,isPet):
	    ...
	    objId = npc.getObjectId()
	    if npcId == KEYMASTER:
		    newAmaskari = self.addSpawn(AMASKARI,19347,253103,-2019,41941,False,0,False,npc.getInstanceId(),0)
