 ...
 */
package quests._195_SecretRitualOfThePriests;
 ...
 */
public class _195_SecretRitualOfThePriests extends Quest
{
	// Instance data
	private static final int INSTANCE_ID = 111;
	
	// Teleport data
	private static final int[] ENTER_COORDS = { -76158, 213412, -7120 }; // enter instance
	private static final int[] EXIT_COORDS = { -12532, 122329, -2984 }; // exit instance
	...
	public _195_SecretRitualOfThePriests(int questId, String name, String descr)
	{
	...
	public String onEvent(String event, QuestState st)
	{
		...
		{
			st.addRewardExpAndSp(52518015, 5817677);
			st.unset("cond");
			st.exitQuest(false);
		...
		switch (st.getState())
		{
			...
			case State.CREATED:
		        if (npcId == CLAUDIA_ATHEBALDT)
		        {
		            ...
		            {
						final QuestState qs = st.getPlayer().getQuestState("194_SevenSignContractOfMammon");
						if (qs != null)
						{
							if (qs.isCompleted())
				...
				case START_DOORS:
					...
					for (L2PcInstance pc : knows)
					{
						if (pc == player && !door.getOpen())
						{
							...
							{
								if (contains(MONSTERS, npc.getNpcId()))
								{
	...
	{
		new _195_SecretRitualOfThePriests(195, "_195_SecretRitualOfThePriests", "Secret Ritual of the Priests");
	}
}
