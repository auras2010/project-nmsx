# Quest 512 Awl Under Foot - Castle - v1.1
...
from l2.universe.gameserver                        import ThreadPoolManager
from l2.universe.gameserver.datatables             import NpcTable
from l2.universe.gameserver.datatables             import SpawnTable
from l2.universe.gameserver.instancemanager        import FortManager
...
from l2.universe.gameserver.model.entity           import Fort
...
def enterInstance(self, player, template, teleto):
	...
	#new instance
        ...
        if not instanceId in self.world_ids:
            ...
            self.saveGlobalQuestVar(self.Castle+"_512_NextEnter", str(System.currentTimeMillis()+14400000))
            raidid = 1
...
def exitInstance(self, npc) :
    if self.worlds.has_key(npc.getInstanceId()):
         ...
         instanceObj = InstanceManager.getInstance().getInstance(world.instanceId)
...
def checkEnter(self, player) :
   entertime = self.loadGlobalQuestVar(self.Castle+"_512_NextEnter")
...
class Quest (JQuest) :
 ...
 def onTalk (self, npc, player):
   ...
   self.Castle = str(npc.getCastle().getCastleId())
   CLAN = player.getClan()
   if CLAN :
