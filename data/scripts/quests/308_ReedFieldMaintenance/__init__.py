...
class Quest (JQuest) :
	...
	def onKill(self,npc,player,isPet) :
		...
		if cond == 1 and npcId in MUCROKIANS :
			if st.getRandom(100) < ExternalConfig.MucrokianHideDropChance :
				st.giveItems(MUCROKIAN_HIDE,1)
				st.playSound("ItemSound.quest_itemget")
		elif cond == 1 and npcId == CHANGED_MUCROKIAN :
			if st.getRandom(100) < ExternalConfig.AwakenedMucrokianHideDropChance :
				st.giveItems(AWAKENED_MUCROKIAN_HIDE,1)
				st.playSound("ItemSound.quest_itemget")
		elif cond == 1 and npcId == CONTAMINATED_MUCROKIAN :
			if st.getRandom(100) < ExternalConfig.ContaminatedMucrokianHideDropChance :
				st.giveItems(MUCROKIAN_HIDE,1)
				st.playSound("ItemSound.quest_itemget")
		return
