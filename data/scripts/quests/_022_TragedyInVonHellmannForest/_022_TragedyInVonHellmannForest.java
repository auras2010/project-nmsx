...
public class _022_TragedyInVonHellmannForest extends Quest
{
	...
	static
	{
		for (int i : new int[] {21553, 21554, 21555, 21556, 21557, 21561})
			MOBS.add(i);
	}
	...
	public _022_TragedyInVonHellmannForest(int questId, String name, String descr)
	{
		...
		questItemIds = new int[] { LOST_SKULL_OF_ELF, REPORT_BOX, SEALED_REPORT_BOX, LETTER_OF_INNOCENTIN, JEWEL_OF_ADVENTURER_RED, JEWEL_OF_ADVENTURER_GREEN };		

	}

	private void spawnGHOST_OF_PRIEST(QuestState st)
	{
		GHOST_OF_PRIESTInstance = st.addSpawn(GHOST_OF_PRIEST, st.getPlayer().getX(),st.getPlayer().getY(),st.getPlayer().getZ(), Rnd.get(50, 100), true, 0);
	}

	private void spawnSOUL_OF_WELL(QuestState st)
	{
		SOUL_OF_WELLInstance = st.addSpawn(SOUL_OF_WELL, st.getPlayer().getX(),st.getPlayer().getY(),st.getPlayer().getZ(), Rnd.get(50, 100), true, 0);
	}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("31334-13.htm"))
		{
			...
			AutoChat(GHOST_OF_PRIESTInstance,"Did you call me, "+st.getPlayer().getName()+"?");
		}
		...
		else if (event.equalsIgnoreCase("31529-12.htm"))
		{
			...
			SOUL_OF_WELLInstance.getAI().setIntention(CtrlIntention.AI_INTENTION_ATTACK,st.getPlayer(),true);
		}
					...
					case INNOCENTIN:
						switch (cond)
						{
							...
							case 18:
								...
								st.addRewardExpAndSp(345966,31578);
								st.unset("cond");
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		{
			if (st.getInt("cond") == 4 && Rnd.get(10) < 1 && st.getQuestItemsCount(LOST_SKULL_OF_ELF) < 1)
			{
	...
	{
		new _022_TragedyInVonHellmannForest(22, "_022_TragedyInVonHellmannForest", "Tragedy In VonHellmann Forest");    	
	}
}
