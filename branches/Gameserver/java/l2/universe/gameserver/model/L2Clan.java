...
import l2.universe.gameserver.instancemanager.TerritoryWarManager.Territory;
...
import l2.universe.gameserver.network.serverpackets.ExSubPledgeSkillAdd;
...
import l2.universe.gameserver.network.serverpackets.PledgeReceiveSubPledgeCreated;
import l2.universe.gameserver.network.serverpackets.PledgeShowInfoUpdate;
import l2.universe.gameserver.network.serverpackets.PledgeShowMemberListAll;
import l2.universe.gameserver.network.serverpackets.PledgeShowMemberListDeleteAll;
...
import l2.universe.gameserver.network.serverpackets.PledgeSkillListAdd;
...
import l2.universe.gameserver.network.serverpackets.PledgeSkillList.SubPledgeSkill;
...
public class L2Clan
{
	...
	public void setNewLeader(L2ClanMember member)
	{
		...
		final L2PcInstance exLeader = getLeader().getPlayerInstance();
		...
		{
			final L2ItemInstance crown = exLeader.getInventory().getItemByItemId(CastleManager.getCrown());
			if (crown.isEquipped())
			{
        ...
        exLeader.updateCLColours(); //m0fire
		exLeader.broadcastUserInfo();
        ...
        newLeader.updateCLColours(); //m0fire
	...
	public String getLeaderName()
	{
		...
		{
			_log.warning("ClanLeader not found for clan: " + _name);
			return "";
		}
	}
	...
	public void addClanMember(L2PcInstance player)
	{
		...
		player.rewardSkills();
		// Update relations when entering a clan
		final Collection<L2PcInstance> plrs = player.getKnownList().getKnownPlayers().values();
		for (final L2PcInstance chars : plrs)
		{
			try
			{
				chars.sendPacket(new RelationChanged(player, player.getRelation(chars), player.isAutoAttackable(chars)));
				if (player.getPet() != null)
					chars.sendPacket(new RelationChanged(player.getPet(), player.getRelation(chars), player.isAutoAttackable(chars)));
			}
	...
	public void removeClanMember(int objectId, long clanJoinExpiryTime)
	{
		...
		if (exMember.isOnline())
		{
			...
			// Update relations when leaving a clan
			final Collection<L2PcInstance> plrs = player.getKnownList().getKnownPlayers().values();
			for (final L2PcInstance chars : plrs)
			{
				try
	...
	*/
	...
	private void storeNotice(String notice, boolean enabled)
	{
		...
		try
		{
			...
			statement.setString(3, enabled ? "true" : "false");
			statement.setString(4, notice);
			statement.setString(5, enabled ? "true" : "false");
			statement.execute();
			statement.close();
		}
		catch (Exception e)
	...
	public void setRankPrivs(int rank, int privs)
	{
		if (_privs.get(rank) != null)
		{
			...
			{
				if (cm.isOnline() && cm.getPowerGrade() == rank)
				{
	...
	public boolean checkAllyJoinCondition(L2PcInstance activeChar, L2PcInstance target)
	{
		...
		{
			switch (targetClan.getAllyPenaltyType())
			{
				case PENALTY_TYPE_CLAN_LEAVED:
					...
					sm = null;
					return false;
				case PENALTY_TYPE_CLAN_DISMISSED:
	...
	public void dissolveAlly(L2PcInstance player)
	{
		...
		setAllyPenaltyExpiryTime(System.currentTimeMillis() + Config.ALT_CREATE_ALLY_DAYS_WHEN_DISSOLVED * 86400000L, L2Clan.PENALTY_TYPE_DISSOLVE_ALLY); //24*60*60*1000 = 86400000
		updateClanInDB();
	...
	public boolean levelUpClan(L2PcInstance player)
	{
		 ...
		 * 2 need more Adena
		 * 3 missing Item
		 * 4 need more Clan Members
		 * 5 need more Clan Reputation
		 * 6 need a Territory
		 */
		byte increaseClanLevel = 7;
		...
		switch (getLevel())
		{
			...
			case 3:
			{
				...
				if (player.getSp() >= 1000000)
				{
					...
					{
						// Item ID 3874 == Alliance Manifesto
			...
			case 4:
			{
				...
				if (player.getSp() >= 2500000)
				{
					...
					{
						// Item ID 3870 == Seal of Aspiration
			...
			case 5:
				...
				{
					if (getMembersCount() >= Config.CLAN_LEVEL_6_REQUIREMENT)
					{
			...
			case 6:
				...
				{
					if (getMembersCount() >= Config.CLAN_LEVEL_7_REQUIREMENT)
					{
			...
			case 7:
				...
				{
					if (getMembersCount() >= Config.CLAN_LEVEL_8_REQUIREMENT)
					{
			...
			case 8:
				...
				{
					if (player.getInventory().getItemByItemId(9910) != null)
					{
						if (getMembersCount() >= Config.CLAN_LEVEL_9_REQUIREMENT)
						{
			...
			case 9:
				...
				{
					if (player.getInventory().getItemByItemId(9911) != null)
					{
						if (getMembersCount() >= Config.CLAN_LEVEL_10_REQUIREMENT)
						{
			...
			case 10:
				...
				if (getReputationScore() >= Config.CLAN_LEVEL_11_COST)
				{
					...
					if (hasTerritory)
					{
						if (getMembersCount() >= Config.CLAN_LEVEL_11_REQUIREMENT)
						{
		...
		if (increaseClanLevel > 0)
		{
			// We send a message saying what is missing for lvling up the clan			
			switch(increaseClanLevel)
			{
				case 1: // SP
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.NOT_ENOUGH_SP));
					break;
				case 2: // Adena
				...
				case 3: // Item
					...
					break;
				case 4: // Members
					// Is there some systemMessage for this?
					player.sendMessage("Not enough clan members");
					break;
				case 5: // Reputation
					player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.THE_CLAN_REPUTATION_SCORE_IS_TOO_LOW));
					break;
				case 6: // Territory
					// Is there some systemMessage for this?
					player.sendMessage("You need a territory");			
					break;
			}
			
			player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.FAILED_TO_INCREASE_CLAN_LEVEL));
			return false;
		}
