...
import l2.universe.gameserver.model.actor.knownlist.NullKnownList;
import l2.universe.gameserver.model.quest.QuestState;
import l2.universe.gameserver.network.SystemMessageId;
import l2.universe.gameserver.network.serverpackets.DropItem;
import l2.universe.gameserver.network.serverpackets.GetItem;
import l2.universe.gameserver.network.serverpackets.InventoryUpdate;
import l2.universe.gameserver.network.serverpackets.SpawnItem;
...
import l2.universe.gameserver.templates.item.L2ItemType;
import l2.universe.gameserver.templates.item.L2Weapon;
import l2.universe.gameserver.util.GMAudit;
...
public final class L2ItemInstance extends L2Object
{
	...
	public void setOwnerId(String process, int owner_id, L2PcInstance creator, Object reference)
	{
		...
		if (Config.LOG_ITEMS)
		{
			if (!Config.LOG_ITEMS_SMALL_LOG || (Config.LOG_ITEMS_SMALL_LOG && getItem().mustBeLogged(getCount())))
			{
	...
	public void changeCount(String process, long count, L2PcInstance creator, Object reference)
	{
		...
		{
			if (!Config.LOG_ITEMS_SMALL_LOG || (Config.LOG_ITEMS_SMALL_LOG && getItem().mustBeLogged(count)))
			{
	...
	/**
	 * Returns if the item is formal wear
	 * 
	 * @return boolean
	 */
	public boolean isFormalWear()
	{
		return getItemId() == 6408;
	}
	...
	{
		return _type2 == L2Item.TYPE2_SHIELD_ARMOR;
	}
	
	public boolean isAccessory() 
	{
		return _type2 == L2Item.TYPE2_ACCESSORY;
	}
	...
	{
		return _item.canBeUsedDuringCombat();
	}
	...
	public static L2ItemInstance restoreFromDb(int ownerId, ResultSet rs) 
	{
		...
		try
		{
			objectId = rs.getInt(1); // do not change it - object Id MUST be 1st field in query
	 ...
	 */
	private synchronized void updateInDb()
    {
		assert _existsInDb;
	...
	/**
	 * Checking if item is only for instance.
	 * 
	 * @return true if item is only for instance.
	 */
	...
	/**
	 * Every instance is contained between ";", and this will solve problems with multi instanced items
	 * 
	 * @return True if item Instance is the same as Player Instance
	 */
	...
	/**
	 * Synerge - Returns true if the player can summon a pet of this level in this instance
	 * Only for pailakas
	 */
	public boolean canSummonPetOnInstance(int charInstanceID, int summonLvl)
	{
		final InstanceWorld world = InstanceManager.getInstance().getWorld(charInstanceID);
		if (world != null)
		{
	...
	/**
	 * Set if its a instance item checking the item ID
	 * Changed now everything to use string comparison. Every instance ID should be separated by a ;
	 * in the beginning and end of the id (; doesnt have a meaning, choosen randomly)
	 * 
	 * @param itemId
	 */
	public void setInstanceItem(int itemId)
	{
		switch (itemId) 
		{
			...
			// Share Items in 3 Pailakas
			case 13032:
			case 13033:
				instanceNr = ";45;44;43;";
				break;
				
			// Not an instance item
			default:
				instanceNr = null;
				break;
		}
