...
import l2.universe.gameserver.model.actor.instance.L2TrapInstance;
...
import l2.universe.gameserver.network.serverpackets.NpcQuestHtmlMessage;
import l2.universe.gameserver.scripting.ManagedScript;
import l2.universe.gameserver.scripting.ScriptManager;
import l2.universe.gameserver.templates.chars.L2NpcTemplate;
import l2.universe.gameserver.templates.item.L2Item;
import l2.universe.gameserver.util.MinionList;
...
public class Quest extends ManagedScript
{
	...
	{
		if ((this.getScriptFile() != null) && this.getScriptFile().getAbsolutePath() != null)
		{
	...
	public L2PcInstance getRandomPartyMember(L2PcInstance player)
	{
		...
		if ((player.getParty() == null) || (player.getParty().getPartyMembers().isEmpty()))
			return player;
		FastList<L2PcInstance> members = new FastList<L2PcInstance>(player.getParty().getPartyMembers());
		Collections.shuffle(members);
		for (L2PcInstance member : members)
		{
			if (player.isInsideRadius(member, 2000, true, false))
				return member;
		}
		return player;
	}
	...
	{
		String content = null;
		if (fileName.startsWith("data/")) // full path already set
			content = HtmCache.getInstance().getHtm(prefix, fileName);
		else
			content = HtmCache.getInstance().getHtm(prefix, "data/scripts/" + getDescr().toLowerCase() + "/" + getName() + "/" + fileName);
		
		if (content == null)
		{
			content = HtmCache.getInstance().getHtm(prefix, "data/scripts/quests/_" + getName() + "/" + fileName);
			if (content == null)
	...
	/**
	 * Window message on client with already completed quest text.
	 * 
	 * @return "This quest have already been completed"
	 */
	protected static String msgQuestCompleted()
	{
		return "<html><body>This quest have already been completed.</body></html>";
	}
	
	/**
	 * Window message on client with not have minimum requirements.
	 * 
	 * @return "You are either not on a quest that involves..."
	 */
	protected static String msgNotHaveMinimumRequirements()
	{
	...
	/**
	 * Window message on client with text that player have too low lvl.
	 * 
	 * @param lvl - player lvl
	 * @return "This quest can only be taken by characters that have a minimum level ..."
	 */
	protected static String msgTooLowLvl(int lvl)
	{
		return "<html><body>This quest can only be taken by characters that have a minimum level of " + lvl + ". Return when you are more experienced.</body></html>";
	}
    ...
    /**
     * Checking if in String array is searching string.
     * 
     * @param s - String array
     * @param ss - searching string
     * @return true if Array contains searching string.
     */
    protected static final boolean contains(String[] s, String ss)
    {
    	for (String ts : s)
    	{
