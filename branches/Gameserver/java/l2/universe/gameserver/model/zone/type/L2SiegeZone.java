...
import l2.universe.gameserver.model.entity.Siegable;
...
public class L2SiegeZone extends L2ZoneType
{
	...
	protected void onExit(L2Character character)
	{
		...
		if (character instanceof L2PcInstance)
		{
			...
			if (_siege != null && _siege instanceof FortSiege && activeChar.getInventory().getItemByItemId(9819) != null)
			{
				// drop combat flag
		...
		{
			/* Synerge - Create a thread to 10 seconds after the exit, to avoid unsummoning on teleport. This causes that the summon
			 * is lost and stucked there
			 */			
			ThreadPoolManager.getInstance().scheduleGeneral(new CheckSummonInSiege((L2SiegeSummonInstance) character), 10000);
		}
	}
	...
	/* 
	 * Synerge - This is a thread to recheck SiegeSummon status, if not in Siege Zone, then unsummon
	 */
	private class CheckSummonInSiege implements Runnable
	{
		private L2Summon _siegeSummon = null;
		
		public CheckSummonInSiege(L2Summon siegeSummon)
		{
			_siegeSummon = siegeSummon;
		}
		
		public void run()
		{
			if (_siegeSummon != null && !_siegeSummon.isInsideZone(L2Character.ZONE_SIEGE))
				_siegeSummon.unSummon(_siegeSummon.getOwner());
		}
	}
}
