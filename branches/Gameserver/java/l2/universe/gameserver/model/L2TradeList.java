...
public class L2TradeList
{
	...
	public static class L2TradeItem
	{
		...
		/**
		 * Synerge - Adding every cost modifier to price
		 * 
		...
		public long getPrice()
		{
			...
			if (getItemId() >= 3960 && getItemId() <= 4026)
				return (long) (_price * Config.RATE_SIEGE_GUARDS_PRICE);
			// Consumables
			else if (_template.isConsumable())
				return (long) (_price * Config.RATE_SHOP_CONSUMABLE_COST);
			// Armors, weapons, jewels. Equipable items
			else if (_template.isEquipable())
				return (long) (_price * Config.RATE_SHOP_EQUIPABLE_COST);
			
			// Every other item on shop
			return (long) (_price * Config.RATE_SHOP_ITEMS_COST);
		}
