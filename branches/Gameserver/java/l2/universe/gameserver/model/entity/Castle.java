...
import l2.universe.gameserver.CastleUpdater;
...
import l2.universe.gameserver.instancemanager.CastleManorManager.CropProcure;
import l2.universe.gameserver.instancemanager.CastleManorManager.SeedProduction;
...
import l2.universe.gameserver.model.zone.type.L2CastleTeleportZone;
...
public class Castle
{
	...
	public void addToTreasury(long amount)
	{
		...
		if (!_name.equalsIgnoreCase("aden") && !_name.equalsIgnoreCase("Rune") && 
				!_name.equalsIgnoreCase("Schuttgart") && !_name.equalsIgnoreCase("Goddard")) 
		{
	...
	public void setOwner(L2Clan clan)
	{
	    if (clan == null)
	    {
	    	_log.log(Level.WARNING, "Could not set owner for castle {} - clan == null: ");
	    	return;
	    }
	    
		// Remove old owner
		if (getOwnerId() > 0 && clan.getClanId() != getOwnerId())
		{
			...
			if (_production != null)
			{
				...
				for (SeedProduction s : _production)
				{
					values[count++] = "(" + getCastleId() + "," + s.getId() + ","
					        + s.getCanProduce() + "," + s.getStartProduce() + "," + s.getPrice()
