...
public class TownWarManager
{
	...
	{
		if (ExternalConfig.TW_AUTO_EVENT)
		{
			isInactive = true;
	...
	public void scheduleEventStart()
	{
		try
		{
			...
			for (String timeOfDay : ExternalConfig.TW_INTERVAL)
			{
		...
		{
			_log.warning("TownWarEngine[TownWarManager.scheduleEventStart()]: Error figuring out a start time. Check TownWarEventInterval in ExternalConfig file.");
		}
	}
	...
	public void starting()
	{
		isInactive = false;
		isStarting = true;
		_task.setStartTime(System.currentTimeMillis() + 60000L * ExternalConfig.TW_TIME_BEFORE_START);
		ThreadPoolManager.getInstance().executeTask(_task);
	}
	...
	public void startEvent()
	{
		isStarting = false;
		isStarted = true;
		if (ExternalConfig.TW_ALL_TOWNS)
		{
		...
		{
			TownManager.getTown(ExternalConfig.TW_TOWN_ID).setIsTWZone(true);
			TownManager.getTown(ExternalConfig.TW_TOWN_ID).updateForCharactersInside();
			Announcements.getInstance().announceToAll("Town War: " + ExternalConfig.TW_TOWN_NAME + " is a war zone.");
		}
		
		_task.setStartTime(System.currentTimeMillis() + 60000L * ExternalConfig.TW_RUNNING_TIME);
		ThreadPoolManager.getInstance().executeTask(_task);
	}
	...
	public void endEvent()
	{
		...
		{
			TownManager.getTown(ExternalConfig.TW_TOWN_ID).setIsTWZone(false);
			TownManager.getTown(ExternalConfig.TW_TOWN_ID).updateForCharactersInside();
			Announcements.getInstance().announceToAll("Town War: " + ExternalConfig.TW_TOWN_NAME + " is returned normal.");
		}
