...
import l2.universe.gameserver.network.serverpackets.RadarControl;
...
public class CTF
{
	...
	public static void AnnounceToPlayers(Boolean toall, String announce)
	{
		...
		{
			final CreatureSay cs = new CreatureSay(0, 2, "", "Announcements : " + announce);
	...
	public static void startJoin(L2PcInstance activeChar)
	{
		...
		if (ExternalConfig.CTF_ANNOUNCE_REWARD)
	...
	public static void teleportStart()
	{
		...
		if (ExternalConfig.CTF_EVEN_TEAMS.equals("SHUFFLE"))
		{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
		{
			public void run()
			{
				...
				for (L2PcInstance player : _players)
				{
					if (player != null)
					{
						if (ExternalConfig.CTF_ON_START_UNSUMMON_PET)
						{
						...
						if (ExternalConfig.CTF_ON_START_REMOVE_ALL_EFFECTS)
						{
	...
	private static boolean startEventOk()
	{
		...
		if (ExternalConfig.CTF_EVEN_TEAMS.equals("NO") || ExternalConfig.CTF_EVEN_TEAMS.equals("BALANCE"))
		{
			if (_teamPlayersCount.contains(0))
				return false;
		}
		else if (ExternalConfig.CTF_EVEN_TEAMS.equals("SHUFFLE"))
		{
	...
	public static void finishEvent()
	{
		...
		if (ExternalConfig.CTF_ANNOUNCE_TEAM_STATS)
		{
	...
	public static void showEventHtml(L2PcInstance eventPlayer, String objectId)
	{
		try
		{
			...
			else if (ExternalConfig.CTF_EVEN_TEAMS.equals("SHUFFLE") && !checkMaxPlayers(_playersShuffle.size()))
			{
				if (!CTF._started)
				{
			...
			else if (eventPlayer.isCursedWeaponEquipped() && !ExternalConfig.CTF_JOIN_CURSED)
			{
	...
	public static boolean addPlayerOk(String teamName, L2PcInstance eventPlayer)
	{
		...
		if (ExternalConfig.CTF_EVEN_TEAMS.equals("NO") || ExternalConfig.CTF_EVEN_TEAMS.equals("SHUFFLE"))
			return true;
		else if (ExternalConfig.CTF_EVEN_TEAMS.equals("BALANCE"))
		{
	...
	public static synchronized void addDisconnectedPlayer(L2PcInstance player)
	{
		 ...
		 */
		if ((ExternalConfig.CTF_EVEN_TEAMS.equals("SHUFFLE") && (_teleport || _started)) 
				|| (ExternalConfig.CTF_EVEN_TEAMS.equals("NO") || ExternalConfig.CTF_EVEN_TEAMS.equals("BALANCE") && (_teleport || _started)))
		{
			if (ExternalConfig.CTF_ON_START_REMOVE_ALL_EFFECTS)
			{
				for (L2Effect e : player.getAllEffects())
	...
	public static void removePlayer(L2PcInstance player)
	{
		if (player._inEventCTF)
		{
			...
			if ((ExternalConfig.CTF_EVEN_TEAMS.equals("NO") || ExternalConfig.CTF_EVEN_TEAMS.equals("BALANCE")) && _players.contains(player))
			{
				setTeamPlayersCount(player._teamNameCTF, teamPlayersCount(player._teamNameCTF) - 1);
				_players.remove(player);
			}
			else if (ExternalConfig.CTF_EVEN_TEAMS.equals("SHUFFLE") 
					&& (!_playersShuffle.isEmpty() && _playersShuffle.contains(player)))
				_playersShuffle.remove(player);
		}
	}
	...
	public static boolean onItemSummon(L2PcInstance player, int itemId)
	{
		...
		if (_players.contains(player))
		{
			...
			/*switch(itemId)
			{
				// this items will be able to summon
				case 12345:
					return true;
				default:
					return false;
