...
public class CharStatus
{
	...
	public void setCurrentHp(double newHp, boolean broadcastPacket)
	{
		...
		final double oldHp = getActiveChar().getCurrentHp();
		...
		// Synerge - Status update for skills with condition <player hp=30> or <player hp=60>.
		if (getActiveChar() instanceof L2PcInstance)
		{
			if (((oldHp > maxHp*0.6) && (_currentHp < maxHp*0.6)) 
					|| (((oldHp < maxHp*0.6) && (_currentHp > maxHp*0.6)))
					|| (((oldHp > maxHp*0.3) && (_currentHp < maxHp*0.3)))
					|| (((oldHp < maxHp*0.3) && (_currentHp > maxHp*0.3))))
			{
				((L2PcInstance) getActiveChar()).broadcastUserInfo();
			}
		}
	}
