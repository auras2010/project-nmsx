 ...
 */
package l2.universe.gameserver.model.actor;

import static l2.universe.gameserver.ai.CtrlIntention.AI_INTENTION_ACTIVE;
import static l2.universe.gameserver.ai.CtrlIntention.AI_INTENTION_FOLLOW;
...
import l2.universe.gameserver.ai.L2AttackableAI;
...
import l2.universe.gameserver.datatables.MapRegionTable.TeleportWhereType;
...
import l2.universe.gameserver.model.ChanceSkillList;
import l2.universe.gameserver.model.CharEffectList;
import l2.universe.gameserver.model.FusionSkill;
import l2.universe.gameserver.model.IChanceSkillTrigger;
...
import l2.universe.gameserver.model.actor.instance.L2ArtefactInstance;
import l2.universe.gameserver.model.actor.instance.L2DoorInstance;
import l2.universe.gameserver.model.actor.instance.L2NpcWalkerInstance;
import l2.universe.gameserver.model.actor.instance.L2OlympiadManagerInstance;
...
import l2.universe.gameserver.model.actor.instance.L2PcInstance.SkillDat;
import l2.universe.gameserver.model.actor.knownlist.CharKnownList;
import l2.universe.gameserver.model.actor.position.CharPosition;
import l2.universe.gameserver.model.actor.stat.CharStat;
import l2.universe.gameserver.model.actor.status.CharStatus;
...
import l2.universe.gameserver.network.serverpackets.Attack;
import l2.universe.gameserver.network.serverpackets.ChangeMoveType;
...
import l2.universe.gameserver.network.serverpackets.MoveToLocation;
import l2.universe.gameserver.network.serverpackets.Revive;
import l2.universe.gameserver.network.serverpackets.ServerObjectInfo;
...
import l2.universe.gameserver.network.serverpackets.TeleportToLocation;
import l2.universe.gameserver.network.serverpackets.FlyToLocation.FlyType;
import l2.universe.gameserver.pathfinding.AbstractNodeLoc;
import l2.universe.gameserver.pathfinding.PathFinding;
import l2.universe.gameserver.skills.AbnormalEffect;
import l2.universe.gameserver.skills.Calculator;
import l2.universe.gameserver.skills.Formulas;
import l2.universe.gameserver.skills.Stats;
import l2.universe.gameserver.skills.effects.EffectChanceSkillTrigger;
import l2.universe.gameserver.skills.funcs.Func;
import l2.universe.gameserver.skills.l2skills.L2SkillAgathion;
import l2.universe.gameserver.skills.l2skills.L2SkillMount;
...
import l2.universe.gameserver.templates.chars.L2CharTemplate;
...
public abstract class L2Character extends L2Object
{
	...
	private boolean _isBluffed = false; // Cannot move/attack until stun timed out	
	...
	private boolean _canExitFromZone = true;
	private int _mustRemainInZoneID = 0;
	...
	public static final byte ZONE_HELLBOUND = 14;
	public static final byte ZONE_NOSTORE = 15;
	public static final byte ZONE_DUNGEON = 16;
	public static final byte ZONE_TOWN = 17;
	public static final byte ZONE_SCRIPT = 18;
	public static final byte ZONE_NOHQ = 19;
	public static final byte ZONE_DANGERAREA = 20;
	public static final byte ZONE_NOBOOKMARK = 21;
	public static final byte ZONE_NOITEMDROP = 22;
	public static final byte ZONE_ALTERED = 23;
	public static final byte ZONE_HEAT_OF_DESERT = 24;
	public static final byte ZONE_FISHING = 25;
	
	private final byte[] _zones = new byte[26];
	protected byte _zoneValidateCounter = 4;
	
	private final List<ScheduledFuture<MagicUseCheckGeoTask>> geoTasks = new CopyOnWriteArrayList<ScheduledFuture<MagicUseCheckGeoTask>>();
	
	private L2Character _debugger = null;
	

	// Polearm Mastery - Max Targets / level
	private final int POLEARM_MASTERY = 216;

	/* Spear Simultaneous Attacks (as described on NcSoft's - updates on high five, but, will use those values for Freya
	    The maximum number of simultaneous attacks when using a Spear is:
     	    No mastery: (5) - 3 on High Five
          Polearm mastery level 8 and below: 10 - 8 on High Five
     	    Polearm mastery level 9 and above: 15 - 13 on High Five

	*/
	private int _maxPolearmAttack;

	public int getMaxPolearmAttacks()
	{
		int poleMasteryLevel = getSkillLevel(POLEARM_MASTERY);
		if (poleMasteryLevel == -1)
			_maxPolearmAttack = 3;	// No Mastery - 5 targets max
		else if (poleMasteryLevel > 0 && poleMasteryLevel <= 8)
			_maxPolearmAttack = 8;
		else if (poleMasteryLevel >= 9)
			_maxPolearmAttack = 13;
		return _maxPolearmAttack;
	}
	...
	{
		if (ExternalConfig.TW_DISABLE_GK && isInTownWarEvent() && !isPendingRevive())
		{
	...
	protected void doAttack(L2Character target)
	{
		...
		if (this instanceof L2PcInstance)
		{
			...
			// Triple zone check just to be sure that we are in a Olympiad Stadium
			if (target.isInsideZone(ZONE_PVP)
					&& target.isInsideZone(ZONE_NOSUMMONFRIEND)
					&& target.isInsideZone(ZONE_NOLANDING)
			...
			if (target.getActingPlayer() != null
					&& ((L2PcInstance) this).getSiegeState() > 0
					&& this.isInsideZone(L2Character.ZONE_SIEGE)
	...
	private boolean doAttackHitByBow(Attack attack, L2Character target, int sAtk, int reuse)
	{
		...
		{
			// Create a new hit task with Medium priority
			// Synerge - Fix for hit animation time, its 60% of sAtk on a mob
			ThreadPoolManager.getInstance().scheduleAi(new HitTask(target, damage1, crit1, miss1, ((L2Attackable) this)._soulshotcharged, shld1), (int)(sAtk*0.6));
		}
		else
	...
	private boolean doAttackHitByPole(Attack attack, L2Character target, int sAtk)
	{
		...
		int attackCountMax = getMaxPolearmAttacks();  			//(int) getStat().calcStat(Stats.ATTACK_COUNT_MAX, 0, null, null);
		int attackcount = 0;
			...
			for (L2Object obj : objs)
			{
				...
				if (obj instanceof L2Character)
				{
					...
					if (this instanceof L2Attackable
							...
							&& ((L2Attackable) this).getEnemyClan() == null
	...
	/**
	 * Stops all currently running MagicUseCheckGeoTasks.
	 */
	private void stopMagicUseCheckGeoTasks()
	{
		while (!geoTasks.isEmpty())
		{
			try
			{
				final ScheduledFuture<MagicUseCheckGeoTask> task = geoTasks.get(0);
				...
				{
					_log.log(Level.WARNING, "Problem while canceling MagicUseCheckGeoTask", ex);
				}
				
				geoTasks.remove(task);
			}
			catch (final IndexOutOfBoundsException ex)
			{
				// list was not empty in the time we checked for emptyness
				// but when we tried to get the first item in the list, the list
				// became empty because being cleared by other thread so we
				// simply end here as our goal was to make the list empty
			}
		}
	}
	...
	private void beginCast(L2Skill skill, boolean simultaneously, L2Character target, L2Object[] targets)
	{
		...
		//Blow skill chance
		CRIT_ATTACK = false;
		if (skill.getSkillType() == L2SkillType.BLOW)
		{
			...
			if (!Formulas.calcPhysicalSkillEvasion(target, skill))
			{
				...
				if (skill.getId() == 30 || (skill.getCondition() & L2Skill.COND_BEHIND) != 0)
					success = (_successChance == 70);
				else if ((skill.getCondition() & L2Skill.COND_CRIT) != 0)
					success = (success && Formulas.calcBlow(this, target, _successChance));
		...
		// to the L2Character AND to all L2PcInstance in the _KnownPlayers of the L2Character
		if (CRIT_ATTACK)
			broadcastPacket(new MagicSkillUse(this, target, displayId, level, hitTime, reuseDelay, 0x20));
		else
		...
		// Synerge - If is casting Seal of Ruler, start protection Task on the artifact
		if (skill.getTargetType() == SkillTargetType.TARGET_HOLY && target instanceof L2ArtefactInstance)
		{
			((L2ArtefactInstance)target).startProtectTask(this);
		}
		...
		if (hitTime > 410)
		{
			...
			MagicUseCheckGeoTask geoT = new MagicUseCheckGeoTask(targets, this, skill, coolTime, simultaneously);
			
			if ((targets.length > 0)
					&& (targets[0] != this)
					&& (Config.LOS_CAST_CHECK_INTERVAL > 0)
					&& !(skill.getSkillType() == L2SkillType.SUMMON_FRIEND))
			{
				stopMagicUseCheckGeoTasks();
				geoTasks.add((ScheduledFuture<MagicUseCheckGeoTask>) ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(geoT, 10, Config.LOS_CAST_CHECK_INTERVAL));
			}
	...
	public boolean doDie(L2Character killer)
	{
		...
		else if (!ExternalConfig.TW_LOSE_BUFFS_ON_DEATH && isInTownWarEvent())
		{
			//null
		}
	...
	{
		return _allSkillsDisabled || isStunned() || isBluffed() || isSleeping() || isParalyzed();
	}
	...
	/**
	 * Synerge - New condition bluff, is the same as stun
	 * @return
	 */
	...
	public void mustRemainInZone(int zoneId, boolean forceTeleport)
	{
		_mustRemainInZoneID = zoneId;
	    _canExitFromZone = false;
	    _isZoneForcedTeleport = forceTeleport;
	}

	public int mustRemainInZone()
	{
		return _mustRemainInZoneID;
	}

	public void canExitFromZone(boolean b)
	{
		_canExitFromZone = b;
	    if (b)
	    	_mustRemainInZoneID = 0;
	}

	public boolean canExitFromZone()
	{
		return _canExitFromZone;
	}
	
	private boolean _isZoneForcedTeleport = false;
	
	public boolean isZoneForcedReturnTeleport()
	{
		return _isZoneForcedTeleport;
	}
	...
	class MagicUseCheckGeoTask implements Runnable
	{
		L2Object[] _targets;
		L2Character _caster;
		L2Skill _skill;
		...
		public MagicUseCheckGeoTask(L2Object[] targets, L2Character currPlayer, L2Skill skill, int coolTime, boolean simultaneously)
		{
		...
		public void run()
		{
			...
			try
			{
				if (_caster.isCastingNow() && _targets != null)
				{
					for (L2Object tgt : _targets)
					{
						if (tgt == null)
							continue;
						...
						if (tgt != _caster)
						{
							if (!GeoData.getInstance().canSeeTarget(_caster, tgt))
							{
								if (_caster instanceof L2PcInstance)
									_caster.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CANT_SEE_TARGET));
								_caster.abortCast();
								_caster.setIsCastingNow(false);
							}
							else if (!Util.checkIfInRange(_skillCastRange, tgt, _caster, true))
							{
								if (_caster instanceof L2PcInstance)
									_caster.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.TARGET_TOO_FAR));
								_caster.abortCast();
								_caster.setIsCastingNow(false);
							}
						}
			...
			{
				_log.log(Level.SEVERE, "Failed executing MagicUseCheckGeoTask.", e);
			}
		}
	}
	...
	/**
	 * Synerge - New condition bluff, same as stuned, but used for now overwriting stun, in cases for example
	 * bluff that turn needs to be stunned or the player will continue hitting without turning, but if stun
	 * is used, onEffectExit will remove stun even if the actual stun landed.
	 */
	public final void startBluffed()
	{
		setIsBluffed(true);
		abortAttack();
		abortCast();
	 ...
	 * is used, onEffectExit will remove stun even if the actual stun landed.
	 */
	public final void stopBluffed(boolean removeEffects)
	{
		if (removeEffects)
			stopEffects(L2EffectType.BLUFF);
	...
	public int getAbnormalEffect()
	{
		...
		if (!isFlying() && (isStunned() || isBluffed()))
	...
	/** Added to manage the multiple hit skills */
	private int _hitToDo;
	@SuppressWarnings("unused")
	private int _hitDone;
	...
	public boolean isInCombat()
	{
		...
		return (ai != null && (ai.getAttackTarget() != null
                        || ai.isAutoAttacking()));
	}
	...
	protected void moveToLocation(int x, int y, int z, int offset)
	{
		...
		final boolean isReturningToSpawn = this instanceof L2Attackable && ((L2Attackable) this).isReturningToSpawnPoint();
			...
			&& !(this instanceof L2NpcWalkerInstance)) // npc walkers not checked
		{
			...
			if (Config.GEODATA == 2 || (this instanceof L2PcInstance && !(isInVehicle && distance > 1500)) || (this instanceof L2Summon && !(this.getAI().getIntention() == AI_INTENTION_FOLLOW)) // assuming intention_follow only when following owner
			...
			if (Config.GEODATA == 2 && originalDistance - distance > 30 && distance < 2000 && !this.isAfraid())
			{
				...
				if ((this instanceof L2Playable && !isInVehicle) || isMinion() || isInCombat())
				{
					...
					if (m.geoPath == null || m.geoPath.size() < 2) // No path found
					{
						...
						{
							teleToLocation(originalX, originalY, originalZ);
							((L2Attackable) this).setisReturningToSpawnPoint(false);
							return;
						}
					...
					{
						// too difficult path for our poor mob
						if (isReturningToSpawn && m.geoPath.size() > 10)
						{
							teleToLocation(originalX, originalY, originalZ);
							((L2Attackable) this).setisReturningToSpawnPoint(false);
							getAI().setIntention(CtrlIntention.AI_INTENTION_IDLE);
	 ...
	 *
	 * @see l2.universe.gameserver.model.L2Character.isInsideRadius(int x, int y, int z, int radius, boolean checkZ, boolean strictCheck)
	 */
		...
		if (!isAttackAborted())
		{
			...
			if (target.isRaid() && target.giveRaidCurse() && !Config.RAID_DISABLE_CURSE)
			{
				if (getLevel() > target.getLevel() + 8)
				{
					 ...
					 */
					final L2Attackable npcTarget = (L2Attackable) target;
					npcTarget.reduceHate(this, 99999);
					if (npcTarget.getTarget() == this)
					{
			...
			{
				final L2PcInstance enemy = (L2PcInstance) target;
			...
			if (!miss && damage > 0)
			{
				...
				if (!isBow && !target.isInvul()) // Do not absorb if weapon is of type bow or target is invulnerable
				{
	...
	public L2Skill removeSkill(int skillId, boolean cancelEffect)
	{
		...
		if (oldSkill != null)
		{
			...
			if (oldSkill instanceof L2SkillMount 
				&& this instanceof L2PcInstance 
			...
			if (oldSkill instanceof L2SkillSummon
					&& oldSkill.getId() == 710
					&& this instanceof L2PcInstance
	...
	public final List<Integer> getAllSkillIds()
	{
		if (_skills == null)
			return null;
	...
	public void onMagicHitTimer(MagicUseTask mut)
	{
		...
		try
		{
			...
			{
				if (((L2Character) tgt).isDead() && (skill.isDebuff() || skill.getSkillType() == L2SkillType.BUFF))
				{
					abortCast();
					return;
				}
			...
			if (this instanceof L2PcInstance)
			{
				final L2PcInstance pcChar = (L2PcInstance) this;
				final int charges = pcChar.getCharges();
				// Check for charges
				...
				if (skill.getNumCharges() > 0)
				{
					if (skill.getMaxCharges() > 0)
						pcChar.increaseCharges(skill.getNumCharges(), skill.getMaxCharges());
					else
						pcChar.decreaseCharges(skill.getNumCharges());
				}
				
				// Consume Souls if necessary
				if (skill.getSoulConsumeCount() > 0 || skill.getMaxSoulConsumeCount() > 0)
				{
					if (!pcChar.decreaseSouls(skill.getSoulConsumeCount() > 0 ? skill.getSoulConsumeCount() : skill.getMaxSoulConsumeCount(), skill))
					{
						abortCast();
						return;
					}
	...
	{
		return isSkillDisabled(skill, false);
	}
	
	public boolean isSkillDisabled(L2Skill skill, boolean isChanceSkill)
	{
		if (skill == null)
			return true;
		
		return isSkillDisabled(skill.getReuseHashCode(), isChanceSkill);
	}
	...
	{
		return isSkillDisabled(reuseHashcode, false);
	}
	
	// Synerge - Add a new condition for chance skills. Chance skills can be cast even with stun, anchor, etc
	public boolean isSkillDisabled(int reuseHashcode, boolean isChanceSkill)
	{
		if ((isAllSkillsDisabled() && !isChanceSkill) || (isChanceSkill && _allSkillsDisabled))
			return true;
		...
		{			
			...
			for (L2Object trg : targets)
			{
				if (trg instanceof L2Character)
				{
									...
									&& getLevel() > targetsCastTarget.getLevel() + 8)))
					{
						 ...
						 * without taking damage, so dont attack petrified players
						 */
						final L2Attackable npcTarget = (L2Attackable) (target.isRaid() ? target : targetsAttackTarget);
						npcTarget.reduceHate(this, 99999);
						if (npcTarget.getTarget() == this)
						{
							npcTarget.abortAttack();
			...
			if (player != null)
			{
				for (L2Object target : targets)
				{
					...
					final L2Character targetChar = (L2Character) target;
					final L2CharacterAI targetAI = targetChar.getAI();
						
					if (skill.isOffensive())
					{
						if (target instanceof L2Playable || target instanceof L2Trap)
						{
							...
							switch (skill.getSkillType())
							{
								...
								default:
									if (target instanceof L2PcInstance)
										targetAI.clientStartAutoAttack();
									else if (target instanceof L2Summon && targetChar.hasAI())
									{
										final L2PcInstance owner = ((L2Summon) targetChar).getOwner();
										if (owner != null)
											owner.getAI().clientStartAutoAttack();
									}
									...
									if (player.getPet() != target && !(this instanceof L2Trap))
										player.updatePvPStatus(targetChar);
							}
						}
						else if (target instanceof L2Attackable)
						{
							switch (skill.getId())
							{
								...
								default:
									...
									targetChar.addAttackerToAttackByList(this);
							}
						}
						...
						if (targetChar.hasAI())
						{
							switch (skill.getSkillType())
							{
								...
								case AGGREMOVE:
									break;
								default:
									targetChar.getAI().notifyEvent(CtrlEvent.EVT_ATTACKED, this);
							}
						}
					}
					else
					{
						if (target instanceof L2Playable)
						{
							...
							if (!(targetPc == this || targetPc == player) && targetPc.getPvpFlag() > 0 || targetPc.getKarma() > 0)
								player.updatePvPStatus();
							
							// If target is in a duel, cancel the duel
							if (targetPc.isInDuel())
							{
								switch (targetPc.getDuelState())
								{
									...
									default:
										...
										if (player.getDuelId() != targetPc.getDuelId())
											targetPc.setDuelState(Duel.DUELSTATE_INTERRUPTED);
								}
							}
						}
						else if (target instanceof L2Attackable)
	...
	{
		if (Config.MOD_CHAMPION_ENABLE && isChampion() && Config.MOD_CHAMPION_HP != 0)
			getStatus().reduceHp(i / Config.MOD_CHAMPION_HP, attacker, awake, isDOT, false);
		else
	...
	{
		if (Config.MOD_CHAMPION_ENABLE && isChampion() && Config.MOD_CHAMPION_MP != 0)
			getStatus().reduceMp(i / Config.MOD_CHAMPION_MP);
		else
	...
	public boolean hasL2InstanceInRadius(final Class<?> clazz, final int radius)
	{
		for (final L2Character character : getKnownList().getKnownCharactersInRadius(radius))
		{
			if (clazz.isInstance(character))
				return true;
		}
		return false;
	}
	...
	/**
	 * Checks whether there is player store/craft in given radius.
	 *
	 * @param radius radius to check
	 *
	 * @return true if there is player store/craft in given radius, otherwise false
	 */
	public boolean hasPcStoreOrCraftInRadius(final int radius)
	{
		for (final L2PcInstance player : getKnownList().getKnownPlayersInRadius(radius))
		{
