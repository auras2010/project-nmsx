...
public class CharKnownList extends ObjectKnownList
{
	...
	public Collection<L2Character> getKnownCharactersInRadius(long radius)
	{
		...
		final L2Character active = getActiveChar();
		final Collection<L2Object> objs = getKnownObjects().values();
		final int x = active.getX();
		final int y = active.getY();
		final int z = active.getZ();
		final int col = active.getTemplate().collisionRadius;
	    final int instanceId = active.getInstanceId();
				
		for (L2Object obj : objs)
		{
			if (obj instanceof L2Character && obj.getInstanceId() == instanceId)
			{
				if (Util.checkIfInRange((int)radius, col, x, y, z, obj, true))
					result.add((L2Character) obj);
			}
		}
		return result;
	...
	// Synerge - Only known ncps and with max height diff
	public Collection<L2Npc> getKnownNpcsInRadius(long radius, int height)
	{
		FastList<L2Npc> result = new FastList<L2Npc>();
		...
		for (L2Object obj : objs)
		{
			...
			{
				if (Util.checkIfInRange((int) radius, getActiveChar(), obj, height))
