...
import l2.universe.gameserver.model.entity.RecoBonus;
...
public class PcStat extends PlayableStat
{
    ...
    // Used to check if the player has gained exp since log in ..added by bot report
	...
	{
		return addExpAndSp(addToExp, addToSp, useVitality, false);
	}
	
	// Synerge - Adding new parameter, is Quest Exp. If true, exp will not be shared with the pet
	public boolean addExpAndSp(long addToExp, int addToSp, boolean useVitality, boolean isQuestExp)
	{				
		...
		// Synerge - If the exp is a reward from a quest, dont share with the pet
		// If this player has a pet that takes from the owner's Exp, give the pet Exp now		
		if (!isQuestExp && activeChar.getPet() instanceof L2PetInstance)
		{
			final L2PetInstance pet = (L2PetInstance) activeChar.getPet();	
			if (Util.checkIfInRange(2000, pet, activeChar, true)) 
			{
				float ratioTakenByPlayer = pet.getPetLevelData().getOwnerExpTaken() / 100f;
	...
	{
		final int val = super.getPAtkSpd();
		
		if (val > Config.MAX_PATK_SPEED && (!getActiveChar().isGM() || !Config.NO_STATS_LIMITS_FOR_GMS))
	...
	{
		final int val = super.getEvasionRate(target);
		
		if (val > Config.MAX_EVASION && (!getActiveChar().isGM() || !Config.NO_STATS_LIMITS_FOR_GMS))
	...
	{
		final int val = super.getMAtkSpd();
		
		if (val > Config.MAX_MATK_SPEED && (!getActiveChar().isGM() || !Config.NO_STATS_LIMITS_FOR_GMS))
