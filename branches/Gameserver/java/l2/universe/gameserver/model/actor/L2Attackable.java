...
import l2.universe.gameserver.ai.L2DefenderAI;
import l2.universe.gameserver.datatables.EventDroplist;
import l2.universe.gameserver.datatables.HerbDropTable;
import l2.universe.gameserver.datatables.ItemTable;
import l2.universe.gameserver.datatables.SkillTable;
import l2.universe.gameserver.datatables.EventDroplist.DateDrop;
...
import l2.universe.gameserver.model.L2Manor;
...
import l2.universe.gameserver.model.actor.knownlist.AttackableKnownList;
import l2.universe.gameserver.model.actor.status.AttackableStatus;
...
public class L2Attackable extends L2Npc
{
	...
	protected void calculateRewards(L2Character lastAttacker)
	{
		...
		try
		{
			...
			//synchronized (getAggroList())
			{
				...
				for (AggroInfo info : getAggroList().values())
				{
					...
					if (damage > 1)
					{
						...
						// Check if dealer isn't too far from this (killed monster)
			...
			// Synerge - Fort guards dont drop if an ally kill him. Just check if is autoAttackable
			if (!(this instanceof L2DefenderInstance) || isAutoAttackable(lastAttacker))
			...
			if (!rewards.isEmpty())
			{
				...
				for (FastMap.Entry<L2Character, RewardInfo> entry = rewards.head(), end = rewards.tail(); (entry = entry.getNext()) != end;)
				{
					...
					{
						attackerPc = (L2PcInstance) attacker;
						attackerParty = attackerPc.getParty();
					}
					else
						return;
					...
					if (attackerPc.getPet() instanceof L2SummonInstance)
						penalty = ((L2SummonInstance) attackerPc.getPet()).getExpPenalty();
					...
					if (attackerParty == null)
					{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							if (attacker instanceof L2PcInstance)
							{
								...
								{
									attackerPc.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.OVER_HIT));
							...
							if (!attacker.isDead())
							{
								...
								if (attacker instanceof L2PcInstance)
								{
									if (attackerPc.getSkillLevel(467) > 0)
									{
										L2Skill skill = SkillTable.getInstance().getInfo(467, attackerPc.getSkillLevel(467));
										if (skill.getExpNeeded() <= addexp)
											attackerPc.absorbSoul(skill, this);
									}
									
									attackerPc.addExpAndSp(addexp, addsp, useVitalityRate());
									if (addexp > 0)
										attackerPc.updateVitalityPoints(getVitalityPoints(damage), true, false);
								}
								else
					...
					else
					{
						...
						// Calculate Exp and SP rewards
						tmp = calculateExpAndSp(levelDiff, partyDmg, true);
						exp_premium = tmp[0];
						sp_premium = tmp[1];
						...
						if (attacker instanceof L2PcInstance)
						{
							if (isOverhit() && attacker == getOverhitAttacker())
							{
								((L2PcInstance) attacker).sendPacket(SystemMessage.getSystemMessage(SystemMessageId.OVER_HIT));
								exp += calculateOverhitExp(exp);
	...
	public void addDamageHate(L2Character attacker, int damage, int aggro)
	{
		...
		// Aggro ai created in L2attackableAI script if not overridden in other AI files
		if (ai == null)
		{
	...
	{
		if (getAI() instanceof L2DefenderAI)
		{
	...
	{
		int dropChance = calculateDropChance(drop, levelModifier, isSweep, lastAttacker.isPremium());
		int itemCount = calculateDropItemCount(drop, dropChance, isSweep, lastAttacker.isPremium());
	...
	private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
		...
		if (lastAttacker.isPremium())
			categoryDropChance *= isRaid() && !isRaidMinion() ? Config.RATE_DROP_ITEMS_BY_RAID : ExternalConfig.PREMIUM_RATE_DROP_ITEMS;
		else
			categoryDropChance *= isRaid() && !isRaidMinion() ? Config.RATE_DROP_ITEMS_BY_RAID : Config.RATE_DROP_ITEMS;
		
		if (Config.MOD_CHAMPION_ENABLE && isChampion())
			categoryDropChance *= Config.MOD_CHAMPION_REWARDS;
		...
		if (Rnd.get(L2DropData.MAX_CHANCE) < categoryDropChance)
		{
			...
			// To get this value simply divide the modified categoryDropChance by the base category chance.
			// This results in a chance that will dictate the drops amounts: for each amount over 100
			// that it is, it will give another chance to add to the min/max quantities.
			
			// For example, If the final chance is 120%, then the item should drop between its min and max one time,
			// and then have 20% chance to drop again.	
			// If the final chance is 330%, it will similarly give 3 times the min and max, and have a 30% chance to give a 4th time.
			// At least 1 item will be dropped for sure. So the chance will be adjusted to 100% if smaller.
			
			int dropChance = calculateDropChance(drop, levelModifier, false, lastAttacker.isPremium());
			if (dropChance < L2DropData.MAX_CHANCE)
				dropChance = L2DropData.MAX_CHANCE;
			
			int itemCount = calculateDropItemCount(drop, dropChance, false, lastAttacker.isPremium());
			if (itemCount > 0)
				return new RewardItem(drop.getItemId(), itemCount);
			else
				return null;
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character mainDamageDealer)
	{
		...
		if (Config.MOD_CHAMPION_ENABLE && isChampion() && (Config.MOD_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE > 0 || Config.MOD_CHAMPION_REWARD_HIGHER_LVL_ITEM_CHANCE > 0))
		{
			int champqty = Rnd.get(Config.MOD_CHAMPION_REWARD_QTY);
			RewardItem item = new RewardItem(Config.MOD_CHAMPION_REWARD_ID, ++champqty);
			
			if (player.getLevel() <= getLevel() && (Rnd.get(100) < Config.MOD_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE))
			{
			...
			else if (player.getLevel() > getLevel() && (Rnd.get(100) < Config.MOD_CHAMPION_REWARD_HIGHER_LVL_ITEM_CHANCE))
			{
				if (Config.AUTO_LOOT || isFlying())
					player.addItem("ChampionLoot", item.getItemId(), item.getCount(), this, true); // Give the item(s) to the L2PcInstance that has killed the L2Attackable
				else
	 ...
	 * @param lastAttacker The L2Character that has killed the L2Attackable
	 */
	public void doEventDrop(L2Character mainDamageDealer)
	{
		if (mainDamageDealer == null)
			return;
	...
	public L2ItemInstance dropItem(L2PcInstance mainDamageDealer, int itemId, int itemCount)
	{
		return dropItem(mainDamageDealer, new RewardItem(itemId, itemCount));
	}
	...
	{
		if (isChampion() && !Config.MOD_CHAMPION_ENABLE_VITALITY)
			return false;
	...
	/**
	 * Synerge - Calculates drop chance, put everything in one function for easy edit
	 * 
	 * @param drop drop data
	 * @param levelModifier level modifier
	 * @param isSweep whether the drop is from sweep
	 * 
	 * @return counted drop chance
	 */
	private int calculateDropChance(final L2DropData drop, final int levelModifier, final boolean isSweep, final boolean isVip)
	{
		...
		if (((!isRaid() && Config.DEEPBLUE_DROP_RULES) || (isRaid() && Config.DEEPBLUE_DROP_RULES_RAID)) && levelModifier > 0)
		{
			// We should multiply by the server's drop rate, so we always get a low chance of drop for deep blue mobs.
			// NOTE: This is valid only for adena drops! Others drops will still obey server's rate
			deepBlueDrop = 3;
		...
		{
			if (isVip && Config.RATE_DROP_ITEMS_ID.get(drop.getItemId()) == 57)
				dropChance *= ExternalConfig.PREMIUM_RATE_DROP_ADENA;
			else
		...
		{
			if (isVip)
				dropChance *= isRaid() && !isRaidMinion() ? Config.RATE_DROP_ITEMS_BY_RAID : ExternalConfig.PREMIUM_RATE_DROP_ITEMS;
			else
				dropChance *= isRaid() && !isRaidMinion() ? Config.RATE_DROP_ITEMS_BY_RAID : Config.RATE_DROP_ITEMS;
		}
		
		if (Config.MOD_CHAMPION_ENABLE && isChampion() && !drop.isAdena() && !drop.isSealStone())
		{
			if (isSweep)
				dropChance *= Config.MOD_CHAMPION_SPOIL_RATE;
			else
				dropChance *= Config.MOD_CHAMPION_REWARDS_RATE;
		}
		...
		return (int) dropChance;
	}
	
	/**
	 * Synerge - Calculating number of items that should be dropped, put everything on 1 function for easy edit
	 * 
	 * @param drop drop data
	 * @param dropChance calculated drop chance
	 * 
	 * @return number of items that should be dropped
	 */
	private int calculateDropItemCount(final L2DropData drop, final int dropChance, final boolean isSweep, boolean isVip)
	{
		// Get min and max Item quantity that can be dropped in one time
		int minCount = drop.getMinDrop();
		int maxCount = drop.getMaxDrop();
		int itemCount = 0;
		int curDropChance = dropChance;
		
		// Count and chance adjustment for high rate servers
		if (curDropChance > L2DropData.MAX_CHANCE && !Config.PRECISE_DROP_CALCULATION)
		{
			final int multiplier = curDropChance / L2DropData.MAX_CHANCE;
			...
			curDropChance %= L2DropData.MAX_CHANCE;
		}
		...
		if (Config.MOD_CHAMPION_ENABLE && isChampion())
		{
			if (drop.isAdena())
			{
				itemCount *= Config.MOD_CHAMPION_ADENAS_REWARDS;
				if (isVip && ExternalConfig.PREMIUM_DROP_ADENA_MULTIPLIER > 1.0)
					itemCount *= ExternalConfig.PREMIUM_DROP_ADENA_MULTIPLIER;
			}
			else if (drop.isSealStone())
				itemCount *= Config.MOD_CHAMPION_SEAL_STONES_REWARDS;
			else if (isSweep)
				itemCount *= Config.MOD_CHAMPION_SPOIL_AMOUNT;
			else
				itemCount *= Config.MOD_CHAMPION_REWARDS_AMOUNT;
		}
		

		if (!Config.MULTIPLE_ITEM_DROP && itemCount > 1 && !ItemTable.getInstance().getTemplate(drop.getItemId()).isStackable())
			itemCount = 1;
	...
	/*
	 *  Synerge - Teleport task to teleport monster back to their positions to avoid being stuck way far from
	 *  their spawn point
	 *  Moving to L2Attackable this, for biger support
	 */
	protected ScheduledFuture<?> _teleTask = null;
	private boolean _isTeleToSpawn = false;
	
	/** Return True if the L2Character already teleporting to spawn. */
	public final boolean isTeleToSpawn()
	{
		return _isTeleToSpawn;
	}
	
	public final void setIsTeleToSpawn(boolean value)
	{
		_isTeleToSpawn = value;
	}
	
	public void teleSpawn()
	{
		setIsTeleToSpawn(true);
		
		/*
		 *  60 second delay before trying to teleport to home. This allows for aggressive mobs to grab a new target
		 *  if one is nearby before porting home.
		 */
		_teleTask = ThreadPoolManager.getInstance().scheduleGeneral(new TeleToSpawn(), 60000);
	}
	
	protected class TeleToSpawn extends Thread
	{
		@Override
		public void run()
		{
			/*
			 *  We perform these checks again because in that delay window, the mob may get killed
			 *  It should also not return if it gets aggro again.
			 */
			if (!isInCombat() && !isAlikeDead() && !isDead())
				teleToLocation(getSpawn().getLocx(), getSpawn().getLocy(), getSpawn().getLocz(), false);
			
			setIsTeleToSpawn(false);
		}
	}
}
