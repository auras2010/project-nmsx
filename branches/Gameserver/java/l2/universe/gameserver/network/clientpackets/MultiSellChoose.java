...
import l2.universe.gameserver.model.L2Augmentation;
...
import l2.universe.gameserver.model.multisell.Entry;
import l2.universe.gameserver.model.multisell.Ingredient;
...
public class MultiSellChoose extends L2GameClientPacket
{
	...
	public void runImpl()
	{
		...
		if (!player.isGM() && !player.isUsingAIOItemMultisell() && (target == null || !list.checkNpcObjectId(target.getObjectId()) || !target.canInteract(player)))
		{
			player.setMultiSell(null);
			return;
		}
		...
		for (Entry entry : list.getEntries())
		{
			if (entry.getEntryId() == _entryId)
			{
				...
				for (Ingredient e : ingredientsList)
				{
					...
					{
						/* Synerge - Adding cost modifier to items needed for this multisell
						 * It only affects stackable items
						 */
						long itemCount = ((Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient()) ? (e.getItemCount() * _amount) : e.getItemCount());
						if (e.isStackable())
							itemCount *= Config.RATE_MULTISELL_ITEMS_COST;
						
						// if this is not a list that maintains enchantment, check the count of all items that have the given id.
						// otherwise, check only the count of items with exactly the needed enchantment level
						if (inv.getInventoryItemCount(e.getItemId(), list.getMaintainEnchantment() ? e.getEnchantLevel() : -1, false) < itemCount)
						{
						...
						return;
							...
							if (Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient())
							{
								...
								if (itemToTake.isStackable())
								{
									 ...
									 * It only affects stackable items
									 */
									if (!player.destroyItem("Multisell", itemToTake.getObjectId(), (long)(e.getItemCount() * _amount * Config.RATE_MULTISELL_ITEMS_COST), player.getTarget(), true))
									{
										player.setMultiSell(null);
										return;
									}
