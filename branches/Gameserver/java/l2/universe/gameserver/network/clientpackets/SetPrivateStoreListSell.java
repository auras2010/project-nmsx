...
public class SetPrivateStoreListSell extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		/**
		 M0fire - Offline shop distance
		 */
		if (ExternalConfig.SHOP_MIN_RANGE_FROM_NPC > 0)
		{
			for (L2Character cha : player.getKnownList().getKnownCharactersInRadius(ExternalConfig.SHOP_MIN_RANGE_FROM_NPC))
			{
		...
		if (ExternalConfig.SHOP_MIN_RANGE_FROM_PLAYER > 0)
       	{
			for (L2Character cha : player.getKnownList().getKnownCharactersInRadius(ExternalConfig.SHOP_MIN_RANGE_FROM_PLAYER))
			{
