...
import l2.universe.gameserver.network.serverpackets.ExAskJoinMPCC;
		...
		if (activeChar.isInParty())
		{
			...
			if (activeParty.getLeader().equals(activeChar))
			{
				...
				else if (activeParty.isInCommandChannel() && !activeParty.getCommandChannel().getChannelLeader().equals(activeChar))
				{
					...
					activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CANNOT_INVITE_TO_COMMAND_CHANNEL));
				}
				else
				{
					//target in a party?
