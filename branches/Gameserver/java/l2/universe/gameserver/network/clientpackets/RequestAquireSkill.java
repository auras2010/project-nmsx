...
import l2.universe.gameserver.model.actor.instance.L2VillageMasterInstance;
	...
	protected void runImpl()
	{
		...
		switch (_skillType)
		{
			case 0:
			{
				if (trainer instanceof L2TransformManagerInstance) // transform skills
				{
					int costid = 0;
					// Skill Learn bug Fix
					final L2TransformSkillLearn[] skillst = SkillTreeTable.getInstance().getAvailableTransformSkills(player);
			...
			case 2:
			{
				...
				cr = SystemMessage.getSystemMessage(SystemMessageId.CLAN_SKILL_S1_ADDED);
				cr.addSkillName(_id);
				player.sendPacket(cr);
				cr = null;
			...
			case 6:
			{
				...
				// Skill Learn bug Fix
				final L2SkillLearn[] skillsc = SkillTreeTable.getInstance().getAvailableSpecialSkills(player);
				for (L2SkillLearn s : skillsc)
				{
					final L2Skill sk = SkillTable.getInstance().getInfo(s.getId(),s.getLevel());
					if (sk == null || sk != skill)
		...
		if (_level > 1)
		{
			...
			for (L2ShortCut sc : allShortCuts)
			{
				...
				{
					final L2ShortCut newsc = new L2ShortCut(sc.getSlot(), sc
