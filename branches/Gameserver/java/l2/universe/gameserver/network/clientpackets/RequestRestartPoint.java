...
public final class RequestRestartPoint extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			Util.handleIllegalPlayerAction(activeChar, "Living player ["+activeChar.getName()+"] called RestartPointPacket! Ban this player!", Config.DEFAULT_PUNISH);
			return;
		}
		...
		if (activeChar.getInstanceId() > 0)
		{
			for (L2ItemInstance i : activeChar.getInventory().getItems())
			{
				if (i.isInstanceItem() && i.isEquipped())
					activeChar.useEquippableItem(i, true);
			}
			
			/* When exiting pailaka zone, unsummon pet */
	        if (activeChar.getPet() != null && (activeChar.getPet().getNpcId() == 14916 || activeChar.getPet().getNpcId() == 14917))
