...
import l2.universe.gameserver.network.serverpackets.EnchantResult;
...
public final class RequestEnchantItem extends AbstractEnchantPacket
{
	...
	protected void runImpl()
	{
		...
		synchronized (item)
		{
			...
			//premium account - enchant chance
			if (ExternalConfig.PREMIUM_ENCH_BONUS && activeChar.isPremium())
			{
				chance += ExternalConfig.PREMIUM_ENCH_CHANCE_BONUS;
			}
			...
			if (Rnd.get(100) < chance)
			{
				...
				// Custom enchant value - see in config
				if (ExternalConfig.ALLOW_CUSTOM_ENCHANT_VALUE)
				{
					item.setEnchantLevel(item.getEnchantLevel() + ExternalConfig.CUSTOM_ENCHANT_VALUE);
				}
				else
				{
