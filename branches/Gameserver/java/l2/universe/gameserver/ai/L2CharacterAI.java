...
import static l2.universe.gameserver.ai.CtrlIntention.AI_INTENTION_CAST;
...
import static l2.universe.gameserver.ai.CtrlIntention.AI_INTENTION_INTERACT;
import static l2.universe.gameserver.ai.CtrlIntention.AI_INTENTION_MOVE_TO;
import static l2.universe.gameserver.ai.CtrlIntention.AI_INTENTION_PICK_UP;
import static l2.universe.gameserver.ai.CtrlIntention.AI_INTENTION_REST;
...
import l2.universe.gameserver.model.L2ItemInstance.ItemLocation;
...
import l2.universe.gameserver.network.serverpackets.AutoAttackStop;
	...
	public boolean canParty(L2Skill sk)
	{
		if (sk.getTargetType() == L2Skill.SkillTargetType.TARGET_PARTY) 
		{
			...
			for (L2Character target : _actor.getKnownList().getKnownCharactersInRadius(sk.getSkillRadius())) 
			{
				...
				L2Npc targets = ((L2Npc) target);
				L2Npc actors = ((L2Npc) _actor);
				if (actors.getFactionId() != null
						&& targets.getFactionId().equals(actors.getFactionId())) 
				{
					count++;
					L2Effect[] effects = target.getAllEffects();
					for (int i = 0; effects != null && i < effects.length; i++) 
