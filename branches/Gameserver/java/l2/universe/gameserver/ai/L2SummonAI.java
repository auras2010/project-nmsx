...
public class L2SummonAI extends L2PlayableAI implements Runnable
{
	...
	{
		/* Trying to avoid if summon near owner. For not flooding with movement, make random check
		 * Also dont move when attacking or casting, only when idle
		 */
		if (!_actor.isAttackingNow()
				&& !_actor.isCastingNow()
				&& !_actor.isDead()
				&& ((L2Summon) _actor).getOwner() != null
