...
/**
 * Synerge - Joined Fortress and Castle Siege AI in one general AI<BR><BR>
 *
 */
public class L2DefenderAI extends L2AttackableAI implements Runnable
{	
	...
	private boolean castleGuard;
	private boolean fortGuard;
	 ...
	 *
	 */
	public L2DefenderAI(L2Character.AIAccessor accessor)
	{
		super(accessor);
		_selfAnalysis.init();
		...
		castleGuard = ((L2DefenderInstance)_actor).getCastle() != null;
		fortGuard = ((L2DefenderInstance)_actor).getFort() != null;
	}
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (!castleGuard && !fortGuard 
				&& (target instanceof L2FortBallistaInstance || target instanceof L2FortCommanderInstance
	...
	private void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			final Collection<L2Character> objs = npc.getKnownList().getKnownCharactersInRadius(_attackRange);
		...
		{
			// Synerge - Add function to teleport guard to its spawn point if its stucked or far away from its spawn
			if (!npc.isTeleToSpawn()
					&& !npc.isDead()
					&& npc.getSpawn() != null
					&& npc.getInstanceId() == 0
					&& !npc.isInsideRadius(npc.getSpawn().getLocx(), npc.getSpawn().getLocy(), npc.getSpawn().getLocz(), 50, true, false))
			{
				npc.teleSpawn();
			}
	...
	private final void factionNotifyAndSupport()
	{
		...
		final L2Npc guard = (L2Npc) _actor;
		
		// Call all L2Object of its Faction inside the Faction Range
		final String faction_id = guard.getFactionId();
		if (faction_id == null)
			return;
		...
		final Collection<L2Character> knowns = _actor.getKnownList().getKnownCharactersInRadius(1000);
		for (L2Character cha : knowns)
		{
			if (cha == null)
				continue;
			...
			if (!(cha instanceof L2Npc))
			{
				if (_selfAnalysis.hasHealOrResurrect && cha instanceof L2PcInstance)
				{
					boolean isDefender;
					if (fortGuard)
						isDefender = guard.getFort().getSiege().checkIsDefender(((L2PcInstance) cha).getClan());
					else
						isDefender = guard.getCastle().getSiege().checkIsDefender(((L2PcInstance) cha).getClan());
	...
	private void attackPrepare()
	{
		...
		if (!_actor.isMuted() && dist_2 > range * range)
		{
			...
			for (L2Skill sk : skills)
			{
				...
				{
					final L2Object OldTarget = _actor.getTarget();
					final L2SkillType skType = sk.getSkillType();
					if (skType == L2SkillType.BUFF || skType == L2SkillType.HEAL)
					{
						boolean useSkillSelf = true;
						switch (skType)
						{
							case HEAL:
								if (_actor.getCurrentHp() > (int) (_actor.getMaxHp() / 1.5))
								{
									useSkillSelf = false;
									break;
								}
			...
			if ((dx * dx + dy * dy > 10000) 
					&& (homeX * homeX + homeY * homeY > 3240000) // 1800 * 1800
					&& _actor.getKnownList().knowsObject(attackTarget))
			{
				// Cancel the target
				_actor.getKnownList().removeKnownObject(attackTarget);
