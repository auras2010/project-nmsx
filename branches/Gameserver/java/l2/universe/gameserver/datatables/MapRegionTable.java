		...
		if (activeChar instanceof L2PcInstance)
		{
					...
					&& !player.isFlying()) // flying players in gracia cant use teleports to aden continent
			{
				...
				if (teleportWhere == TeleportWhereType.SiegeFlag)
				{
					...
					final L2SiegeFlagInstance tw_flag = TerritoryWarManager.getInstance().getFlagForClan(player.getClan());
					if (tw_flag != null)
		    ...
		    if (player.isInsideZone(L2Character.ZONE_HELLBOUND))
		          return new Location(-16443, 208811, -3665);
			
			// Checking if in arena
			final L2ArenaZone arena = ZoneManager.getInstance().getArena(player);
			if (arena != null)
