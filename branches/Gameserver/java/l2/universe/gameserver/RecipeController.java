...
import l2.universe.gameserver.model.L2RecipeInstance;
import l2.universe.gameserver.model.L2RecipeList;
import l2.universe.gameserver.model.L2RecipeStatInstance;
...
import l2.universe.gameserver.network.serverpackets.RecipeBookItemList;
import l2.universe.gameserver.network.serverpackets.RecipeItemMakeInfo;
import l2.universe.gameserver.network.serverpackets.RecipeShopItemInfo;
...
public class RecipeController
{
	...
	private static class RecipeItemMaker implements Runnable
	{
		...
		private int _materialsRefPrice;
		...
		public RecipeItemMaker(L2PcInstance pPlayer, L2RecipeList pRecipeList, L2PcInstance pTarget)
		{
			...
			{
				set_materialsRefPrice(get_materialsRefPrice() + i.getReferencePrice() * i.getQuantity());
		...
		public void run()
		{
			...
			if (Config.ALT_GAME_CREATION && !_items.isEmpty())
			{
				...
				if (!_items.isEmpty())
				{
					...
					_delay = (int) (Config.ALT_GAME_CREATION_SPEED * _player.getMReuseRate(_skill) * GameTimeController.TICKS_PER_SECOND / Config.RATE_CRAFT_CONSUMABLE_COST)
		...
		private List<TempItem> listItems(boolean remove)
		{
			...
			{
				int quantity = _recipeList.isConsumable() ? (int) (recipe.getQuantity() * Config.RATE_CRAFT_CONSUMABLE_COST) : recipe.getQuantity();				
				if (quantity > 0)
				{
		...
		/**
		 * @return the _materialsRefPrice
		 */
		public int get_materialsRefPrice()
		{
			return _materialsRefPrice;
		}

		/**
		 * @param _materialsRefPrice the _materialsRefPrice to set
		 */
		public void set_materialsRefPrice(int _materialsRefPrice)
		{
			this._materialsRefPrice = _materialsRefPrice;
		}
	}
