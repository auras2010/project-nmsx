...
public class CrystalCaverns extends Quest
{
	...
	public String onSkillSee (L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isPet)
	{
		...
		else if (npc.getNpcId() == TEARS && skill.getId() == 2369 && caster != null)
		{
			InstanceWorld tmpworld = InstanceManager.getInstance().getWorld(npc.getInstanceId());
			if (tmpworld instanceof CCWorld)
			{
				...
				if (world.dragonScaleNeed == 0 && Rnd.get(100) < 80)
				{
					...
					npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "ah... I am so vulnerable..."));
				}
			}
			else
				_log.info("***** Crystal Cavern, " + caster.getName() + ": Try to cheat Tears instance.");
		}
		...
		if (npc.getNpcId() == TEARS)
		{
			...
			if (tmpworld instanceof CCWorld)
			{
				...
				if (nowHp < maxHp * 0.4 && rand < 5)
				{
					...
					for (int i = 0; i < 10; i++)
					{
						L2Npc copy = addSpawn(TEARS_COPY, npc.getX(), npc.getY(), npc.getZ(), 0, true, 0, false, attacker.getInstanceId());
						copy.setRunning();
				...
				else if (!world.isUsedInvulSkill && (nowHp < (maxHp * 0.1)) && (rand > 994))
				{
					world.isUsedInvulSkill = true;
					npc.setIsInvul(true);
					npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "hahaha... Now you cannot kill me..."));
				}
			}
		}
		return null;
	...
	public String onTalk (L2Npc npc, L2PcInstance player)
	{
		...
		return "Cannot visit Baylor now.";
	}
