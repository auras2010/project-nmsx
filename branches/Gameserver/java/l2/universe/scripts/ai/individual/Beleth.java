package l2.universe.scripts.ai.individual;
...
public class Beleth extends L2AttackableAIScript
{
	...
	private static final int[] RING = { 10314, 1 }; // {itemId, count)
	
	// Chamber of Beleth Zone ID
	private static final int BELETHZONEID = 50501;
	
	// NPC Id's
	private static final int BELETH = 29118; // Beleth.
	private static final int CLONE = 29119; // Beleth's clone.
	private static final int[] INCARNATION = { 29126, 29127 }; // Beleth's incarnations.
	...
	private static final int CUBIC = 32376; // Start npc - Beleth's Slave
	...
	// Time before door to beleth opens. Default: 5 minutes.
	private static final int _doorWaitTimeDuration = 300000;
	
	// Time before beleth spawns (since door opens) Default: 5 minutes.
	private static final int _spawnWaitTimeDuration = 300000;
	...
	// Camera locations
	private static final int[] camX = { 16194, 16568, 16896, 16325, 16327, 16325, 16073 };
	private static final int[] camY = { 212718, 212897, 213183, 214614, 213135, 214101, 213738 };
	
	private static final int[] VORTEXSPAWN = { 16325, 214983, -9353 };
	private static final int[] COFFSPAWN = { 12471, 215602, -9360, 49152 };
	private static final int[] EXITLOC = { -24095, 251617, -3374 };
		...
		public void run()
		{
			switch (_event)
			{
				...
				case start:
					...
					_chamberOfBeleth.broadcastPacket(new ExShowScreenMessage(1, -1, 2/*TOP CENTER*/, 0, 0/*FONT SIZE NORMAL*/, 0, 0, true, 10000, 0, "The door to Beleth will open in 5 minutes."));
					ThreadPoolManager.getInstance().scheduleGeneral(new eventExecutor(Event.open_door, null), debug ? 30000 : _doorWaitTimeDuration);
					break;
				case open_door:
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new eventExecutor("camera_2", null), debug ? 30000 : _spawnWaitTimeDuration);
					break;
				case beleth_spawn:
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new eventExecutor(Event.beleth_despawn, null), 300000);
					new eventExecutor("camera_0", temp).run();
					break;
				case close_door:
					DoorTable.getInstance().getDoor(DOOR).closeMe();
					new eventExecutor(Event.clone_spawn, null).run();
					break;
				case clone_spawn:
					...
					new eventExecutor(Event.beleth_disappear, null).run();
					break;
				case beleth_disappear:
					...
					for (L2Npc npc : _npcList)
						npc.deleteMe();
					_npcList.clear();
					new eventExecutor(Event.mobilize_clones, null).run();
					break;
				...
				case respawn_clone:
					...
					break;
				case respawn_clones:
					if (GrandBossManager.getInstance().getBossStatus(BELETH) == DEAD) //if beleth is dead, dont respawn clones
						break;
				...
				case spawn_extras:
					...
					break;
				case beleth_dead:
					new eventExecutor(Event.death_camera, _npc).run();
					...
					new eventExecutor(Event.clone_despawn, null).run();
					break;
				case death_camera:
				...
				case beleth_despawn:
					...
					if (_lastAction + despawnDelay < System.currentTimeMillis())
					{
						...
						// others.
						if (GrandBossManager.getInstance().getBossStatus(BELETH) == ALIVE)
							new eventExecutor(Event.beleth_unlock, null).run();
					}
					else
					{
						long time = _lastAction + despawnDelay - System.currentTimeMillis();
						if (time >= 0)
							ThreadPoolManager.getInstance().scheduleGeneral(new eventExecutor(Event.beleth_despawn, null), time);
					}
					break;
				case clone_despawn:
					if (_clones.isEmpty() || _clones.size() == 0)
	...
	private void handleCamera(L2Npc npc, int cameraId)
	{
		...
		switch (cameraId)
		{
			...
			case 2: // Camera 2 - Beleth appearance
				npc = addSpawn(CAMERA, camX[0], camY[0], locZ, 0, false, 0);
				_npcList.add(npc);
	...
	private void initSpawnLocs()
	{
		...
		for (int i = 0; i < 16; i++)
		{
			if (i % 2 == 0)
				...
				radius += 50;
			_cloneLoc[i] = new Location(centerX + (int) (radius * Math.sin(i * angle)), centerY + (int) (radius * Math.cos(i * angle)), Util.convertDegreeToClientHeading(270 - i * 22.5));
		}
		// Outer clone square
		radius = 1340;
		...
		for (int i = 0; i < 16; i++)
		{
			...
			if ((i + 2) % 4 == 0)
				addH++;
			_cloneLoc[i + 16] = new Location(centerX + (int) (radius * decX * mulX), centerY + (int) (radius * decY * mulY), Util.convertDegreeToClientHeading(180 + addH * 90));
		}
	...
	{
		_chamberOfBeleth.broadcastPacket(new MagicSkillUse(npc, npc, skillId, 1, hittime, 0));
	}
	...
	{
		new eventExecutor(event, npc).run();
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (npcId == CUBIC)
		{
			...
			if (GrandBossManager.getInstance().getBossStatus(BELETH) == DEAD)
				return "<html><body>Beleth is dead! </html></body>";
			if (checkConditions(player))
			{
				new eventExecutor("start", null).run();
				if (debug)
				...
				else
				{
					for (L2PcInstance member : player.getParty().getCommandChannel().getMembers())
					{
						if (member != null)
						{
							_chamberOfBeleth.allowPlayerEntry(member, 7200);
							member.setInstanceId(0);
							member.teleToLocation(16320, 209481, -9356, true);
							if (member.getPet() != null)
							{
								member.getPet().setInstanceId(0);
								member.getPet().teleToLocation(16320, 209481, -9356, true);
							}
						}
					}
				}
		...
		else if (npcId == COFFIN)
		{
			if (debug)
			{
				L2ItemInstance ring = player.getInventory().addItem("Beleth", RING[0], RING[1], player, npc);
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		if (npcId == CLONE)
		{
			...
			ThreadPoolManager.getInstance().scheduleGeneral(new eventExecutor("respawn_clone", npc), 60000);
		}
		else if (npcId == BELETH)
			new eventExecutor("beleth_dead", npc).run();
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet, L2Skill skill)
	{
		...
		if (npcId == BELETH)
		{
			if (_spawnTrigger && System.currentTimeMillis() > _spawnTime + _spawnInterval)
				new eventExecutor("respawn_clones", npc).run();
		}
		if ((npcId == BELETH) && (npc.getCurrentHp() <= (npc.getMaxHp() * _relocatePercent)) && (_relocatePercent > 0))
		{
			...
			{
				_spawnTrigger = true;
				new eventExecutor("spawn_ring", npc).run();
				new eventExecutor("respawn_clones", npc).run();
			}
			else if (_relocatePercent >= 0.2 && _relocatePercent < 0.4)
			{
				new eventExecutor("spawn_extras", npc).run();
				new eventExecutor("respawn_clones", npc).run();
			}
			new eventExecutor("relocate_beleth", npc).run();
	...
	public Beleth(int id, String name, String descr)
	{
		...
		// Add mobs
		int[] mobs = { BELETH, CLONE };
		registerMobs(mobs, QuestEventType.ON_ATTACK, QuestEventType.ON_KILL, QuestEventType.ON_SKILL_SEE);
