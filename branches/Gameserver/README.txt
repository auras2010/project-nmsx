NOTE: This short guide is for a L2jInfinity Server. http://L2j-Infinity,info

If you received this file as a part of a packaged or bundled build:
 
L2jInfinity only supports L2jInfinity software obtained directly from L2jInfinity sources.

What this means is, if you obtained L2jInfinity from a source other than our SVN or nightly distribution,
...
WARNING: L2jInfinity Server is not a plug and play type of server. 
...
L2jInfinity is an Alternative Lineage 2 Game Server written in pure Java for
best compatibility. L2jInfinity gives you the possibility to legally host a game
server for this popular Korean MMO created by NCSoft. It is still
unfinished and many features are missing, but L2jInfinity Dev team is working
hard on implementing them. L2jInfinity Server is distributed under the terms
...
Whereas L2jInfinity is distributed under the terms of GNU/GPL, we require you to:
a) Preserve login notice. This gives us, L2jInfinity Developers, appropriate
...
the same archive. NO world content should be incorporated in L2jInfinity
distribution.
...
They contain all the info you need to setup and run a L2jInfinity Server.

This Server distribution does not contain any spawn/drop data or any world
content. (Datapack)

L2jInfinity Server has also a possibility to change exp/sp/drop rates relative
...
To make someone an admin you need to edit, in the L2jInfinitydb database, the desired character in the 
...
Website: http://L2j-Infinity,info



Please note that L2jInfinity Devs can't help players with connecting issues or
anything related to playing on private servers. If you can't connect,
you should contact your server GM's. We can solve only L2jInfinity server
~software~ related issues. We don't have any backdoors or anything
that would enable us GM accounts on every server using L2jInfinity, so there's
...
programming skills are not always required as L2jInfinity needs much more than
java code.
...
accepted, you might be invited to join L2jInfinity Dev Team.
...
If you are not sure if it should be reported here, make a post about it in the L2jInfinity forum.
...
Please do NOT report bugs related to unofficial add-ons to L2jInfinity. L2jInfinity
...
Have fun playing L2jInfinity ;)
Dev Team

    L2jInfinity Server, Copyright (C) 2008-2010 
    L2jInfinity Server comes with ABSOLUTELY NO WARRANTY.
