...
#Chances
CHANCE = 36
SPECIAL = int(Config.RATE_QUEST_REWARD) * 5
#NPCs
GILMORE = 30754
RODEMAI = 30756
ORVEN = 30857
...
def rewards(st,npcId):
    ...
    if npcId == ORVEN and st.getQuestItemsCount(CRUCIFIX) :
       ...
       state=True
       if chance < 50 :
          st.giveRewardQIBased(1875, 1)
       elif chance < 70 :
          st.giveRewardQIBased(952, 5)
       else :
          st.giveRewardQIBased(2437, 1)
    elif npcId == GARVARENTZ and st.getQuestItemsCount(OLD_TOTEM) :
       st.set("mission","2")
       st.takeItems(OLD_TOTEM,-1)
       state=True
       if chance < 45 :
          st.giveRewardQIBased(1882, 70)
       elif chance < 95 :
          st.giveRewardQIBased(1881, 50)
       else :
          st.giveRewardQIBased(191, 1)
    elif npcId == KAIEN and st.getQuestItemsCount(OLD_HILT) :
       st.set("mission","3")
       st.takeItems(OLD_HILT,-1)
       state=True
       if chance < 50 :
          st.giveRewardQIBased(1874, 25)
       elif chance < 75 :
          st.giveRewardQIBased(1887, 1)
       elif chance < 99 :
          st.giveRewardQIBased(951, 1)
       else :
          st.giveRewardQIBased(133, 1)
    elif npcId == RODEMAI and st.getQuestItemsCount(OLD_KEY) :
       st.set("mission","4")
       st.takeItems(OLD_KEY,-1)
       state=True
       if chance < 40 :
          st.giveRewardQIBased(1879, 55)
       elif chance < 90 :
          st.giveRewardQIBased(951, 1)
       else :
          st.giveRewardQIBased(885, 1)
    return state
...
class Quest (JQuest) :
 ...
 def onEvent (self,event,st) :
     ...
     elif (event == "30754-06.htm" or event == "30754-16.htm") and cond == 1 :
       if not amount :
          htmltext = "30754-06a.htm"
       else:
          if st.getRandom(100) < SPECIAL :
            htmltext="30754-10.htm"
            st.set("ok","1")
            st.set("amount",str(amount))
          else:
            st.giveRewardQIBased(57, amount * 60)
          st.takeItems(ARTICLES_DEAD_HEROES,-1)
     elif event == "30754-11.htm" and cond == 1 :
         if st.getInt("ok") != 1:
            htmltext=default
         else :
            if st.getRandom(100) < SPECIAL : # this is a guess
                st.set("cond","2")
                st.unset("ok")
 ...
 def onTalk (self,npc,player):
     ...
     if id == State.CREATED : 
       ...
       if npcId==GILMORE:
         ...
         if amt:
            st.unset("amount")
            st.giveRewardQIBased(57, amt*50+bonus)
 ...
 def onKill(self,npc,player,isPet):
     ...
     if st.getInt("cond") == 1 :
         npcId = npc.getNpcId()
         st.dropQuestItems(ARTICLES_DEAD_HEROES, 0, 1, 0, True, CHANCE+(npcId-20234)*2, True)
     return
