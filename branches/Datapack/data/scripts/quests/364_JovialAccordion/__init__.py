# Made by Mr. Have fun! Version 0.2
# fixed by Elektra and Rolarga Version 0.3
# fixed by Mr and Drlecter
import sys
from l2.universe.gameserver.model.quest import State
from l2.universe.gameserver.model.quest import QuestState
from l2.universe.gameserver.model.quest.jython import QuestJython as JQuest

qn = "364_JovialAccordion"

KEY_1 = 4323
KEY_2 = 4324
BEER = 4321
ECHO = 4421
CLOTHES = 4322

class Quest (JQuest) :

 def __init__(self,id,name,descr):
     JQuest.__init__(self,id,name,descr)
     self.questItemIds = [KEY_1, KEY_2, BEER, CLOTHES]

 def onEvent (self,event,st) :
    htmltext = event
    if event == "30959-02.htm" :
        st.set("cond","1")
        st.setState(State.STARTED)
        st.playSound("ItemSound.quest_accept")
    elif event == "30957-02.htm" :
        st.set("cond","2")
        st.giveItems(KEY_1,1)
        st.giveItems(KEY_2,1)
    elif event == "30961-03.htm" :
      if st.getQuestItemsCount(KEY_1) :
        n = st.getRandom(100)
        if n >= 50:
            st.takeItems(KEY_1,1)
            st.giveItems(CLOTHES,1)
            htmltext = "30961-OPEN.htm"
        else:
            st.takeItems(KEY_1,1)
            st.set("fail1","1")
            htmltext = "30961-FAIL.htm"
    elif event == "30960-03.htm" :
      if st.getQuestItemsCount(KEY_2) :
        m = st.getRandom(100)
        if m <= 50 :
            st.takeItems(KEY_2,1)
            st.giveItems(BEER,1)
            htmltext = "30960-OPEN.htm"
        else:
            st.takeItems(KEY_2,1)
            st.set("fail","1")
            htmltext = "30960-FAIL.htm"
    return htmltext

 def onTalk (self,npc,player):
   htmltext = "<html><body>You are either not on a quest that involves this NPC, or you don't meet this NPC's minimum quest requirements.</body></html>"
   st = player.getQuestState(qn)
   if not st : return htmltext

   npcId = npc.getNpcId()
   id = st.getState()
   if npcId != 30959 and id != State.STARTED : return htmltext

   if id == State.CREATED :
     st.set("cond","0")
     st.set("ok","0")
     st.set("ok1","0")
     st.set("fail","0")
     st.set("fail1","0")
   cond=st.getInt("cond")
   if npcId == 30959 :
     if cond == 0 :
       if player.getLevel() >= 15 :
         htmltext = "30959-01.htm"
       else :
         htmltext = "30959-00.htm"
         st.exitQuest(1)
     elif cond == 3 :
        st.playSound("ItemSound.quest_finish")
        st.giveReward(ECHO, 1)
        st.exitQuest(1)
        htmltext = "30959-03.htm"
     elif cond == 4 :
        st.playSound("ItemSound.quest_finish")
        st.exitQuest(1)
        htmltext = "30959-04.htm"
     elif cond >= 1 :
        htmltext = "30959-02.htm"
   elif npcId == 30957 :
     if cond == 1 :
        htmltext = "30957-01.htm"
     elif cond == 2 and not st.getQuestItemsCount(KEY_1) and not st.getQuestItemsCount(KEY_2) and st.getInt("ok") and st.getInt("ok1") :
        st.set("cond","3")
        st.giveItems(57,100)
        htmltext = "30957-06.htm"
     elif cond == 2 and not st.getQuestItemsCount(KEY_1) and not st.getQuestItemsCount(KEY_2) and st.getInt("ok1") :
        st.set("cond","3")
        htmltext = "30957-04.htm"
     elif cond == 2 and not st.getQuestItemsCount(KEY_1) and not st.getQuestItemsCount(KEY_2) and st.getInt("ok"):
        st.set("cond","3")
        htmltext = "30957-04.htm"
     elif cond == 2 and st.getInt("fail") and st.getInt("fail1"):
        st.set("cond","4")
        htmltext = "30957-07.htm"
     elif cond == 3 :
        htmltext = "30957-05.htm"
     elif cond == 2 :
        htmltext = "30957-03.htm"
   elif npcId == 30960 and cond :
        htmltext = "30960-01.htm"
   elif npcId == 30961 and cond :
        htmltext = "30961-01.htm"
   elif npcId == 30060 and st.getQuestItemsCount(BEER):
        st.set("ok","1")
        st.takeItems(BEER,1)
        htmltext = "30060-01.htm"
   elif npcId == 30075 and st.getQuestItemsCount(CLOTHES):
        st.set("ok1","1")
        st.takeItems(CLOTHES,1)
        htmltext = "30075-01.htm"
   return htmltext

QUEST       = Quest(364,qn,"Jovial Accordion")

QUEST.addStartNpc(30959)

for npcId in [30959,30957,30060,30961,30960,30075]:
  QUEST.addTalkId(npcId)
