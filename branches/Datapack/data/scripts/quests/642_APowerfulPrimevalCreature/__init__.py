...
DINOSAUR_EGG = 8775
DINOSAURS = [22196,22197,22198,22199,22200,22201,22202,22203,22204,22205,22218,22219,22220,22223,22224,22225,18344,22742,22743,22744,22745]
REWARDS = [8690,8692,8694,8696,8698,8700,8702,8704,8706,8708,8710]
REWARDSDYN = [ 9969,9967,9970,9975,9972,9971,10545,10544,9973,9974,9968 ]
...
class Quest (JQuest) :
 ...
 def onAdvEvent (self,event,npc, player) :
    ...
    elif event == "32105-09.htm" :
       if count_tissue >= 450 :
          htmltext = "32105-09.htm"
          st.takeItems(DINOSAUR_TISSUE,450)
          st.giveItems(REWARDSDYN[st.getRandom(len(REWARDSDYN))],1)
       else :
          htmltext = "Incorrect item count"
    return htmltext
    ...
    if st :
       if st.getState() == State.STARTED :
          ...
          if cond == 1 :
             if npcId == 18344 :
                itemId = DINOSAUR_EGG
                chance = ExternalConfig.APowerfulPrimevalCreatureEggDropChance
                numItems, chance = divmod(chance,100)
             else :
                itemId = DINOSAUR_TISSUE
                chance = ExternalConfig.APowerfulPrimevalCreatureTissueDropChance
                numItems, chance = divmod(chance,100)
