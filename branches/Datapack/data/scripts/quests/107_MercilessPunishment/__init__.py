...
from l2.universe.gameserver.network.serverpackets      import SocialAction
      ...
      if player.getRace().ordinal() != 3 : 
        ...
        st.exitQuest(1) 
          ...
          if st.getInt("id") != 107 : 
            ...
            st.giveReward(LESSER_HEALING_ID, 100)
            st.giveReward(BUTCHER_ID, 1)
            st.giveReward(CRYSTAL_BATTLE, 10)
            st.giveReward(CRYSTAL_LOVE, 10)
            st.giveReward(CRYSTAL_SOLITUDE, 10)
            st.giveReward(CRYSTAL_FEAST, 10)
            st.giveReward(CRYSTAL_CELEBRATION, 10)
            # check the player state against this quest newbie rewarding mark.
            newbie = player.getNewbie()
            if newbie | NEWBIE_REWARD != newbie :
               player.setNewbie(newbie|NEWBIE_REWARD)
               if player.getClassId().isMage() :
                  st.giveReward(SPIRITSHOT_NO_GRADE_FOR_BEGINNERS, 3000)
                  st.playTutorialVoice("tutorial_voice_027")
               else :
                  st.giveReward(SOULSHOT_NO_GRADE_FOR_BEGINNERS, 7000)
                  st.playTutorialVoice("tutorial_voice_026")
               player.sendPacket(ExShowScreenMessage("Acquisition of race-specific weapon complete. Go find the Newbie Guide.",3000))
            st.unset("cond") 
