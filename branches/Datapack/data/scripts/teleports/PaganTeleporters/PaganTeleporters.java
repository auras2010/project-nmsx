	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		switch (npc.getNpcId())
		{
			case 32034:
				...
				{
					st.takeItems(VISITOR_MARK, 1); // This part must happen when u walk through doors >.<
			...
			case 32039:
				if (player.getLevel() < 73
						|| (st.getQuestItemsCount(VISITOR_MARK) == 0 && st.getQuestItemsCount(FADED_VISITOR_MARK) == 0 && st.getQuestItemsCount(PAGAN_MARK) == 0))
				{
					htmltext = msgNotHaveMinimumRequirements();
					st.exitQuest(true);
				}
				else
				{
