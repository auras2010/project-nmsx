...
public class IceFairySirra extends L2AttackableAIScript
{
	...
	{
		super(id, name, descr);
		final int[] mob = { STEWARD, 22100, 22102, 22104, 29056 };
		registerMobs(mob);
		...
		final String test = loadGlobalQuestVar("Sirra_Respawn");
		if (!test.equalsIgnoreCase(""))
		{
			final long remain = Long.parseLong(test) - System.currentTimeMillis();
			if (remain <= 0)
				init(false);
			else
			{
	...
	public void init(boolean isBusy)
	{
		...
		final L2Npc steward = findTemplate(STEWARD);
		if (steward != null)
	...
	{
		final int[][] mobs = { { 29060, 105546, -127892, -2768 }, { 29056, 102779, -125920, -2840 }, { 22100, 111719, -126646, -2992 }, { 22102, 109509, -128946, -3216 }, { 22104, 109680, -125756, -3136 } };
		L2Spawn spawnDat;
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		if (npc.getNpcId() == 29056)
		{
			...
			_allMobs.clear();
			init(true);
			final int respawnMinDelay = 43200000 * (int) (Config.RAID_MIN_RESPAWN_MULTIPLIER);
			final int respawnMaxDelay = 129600000 * (int) (Config.RAID_MAX_RESPAWN_MULTIPLIER);
			final int respawn_delay = Rnd.get(respawnMinDelay, respawnMaxDelay);
