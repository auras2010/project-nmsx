...
import l2.universe.gameserver.network.serverpackets.MoveToPawn;
 ...
 *       vortex
 * 
 * @author Fulminus version 0.1, Synerge
 */
public class Baium extends L2AttackableAIScript
{
	...
	public Baium(int questId, String name, String descr)
	{
		...
		else if (status == AWAKE)
		{
			...
			_baium = (L2GrandBossInstance) addSpawn(LIVE_BAIUM, loc_x, loc_y, loc_z, heading, false, 0);
			GrandBossManager.getInstance().addBoss(_baium);
			ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
			{
				...
				public void run()
				{
					...
					{
						_log.warning("Could not wake up baium: ");
					}
				}
			}, 100L);
		}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("baium_wakeup") && npc != null)
		{
			if (npc.getNpcId() == LIVE_BAIUM)
			{
				...
				// Send the archangels to attack the wake character
				for (final L2Attackable minion : _Minions)
				{
					if (player != null)
					{
						minion.getAI().notifyEvent(CtrlEvent.EVT_ATTACKED, player);
						minion.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, player, 500);
					}
				}
		...
		else if (event.equalsIgnoreCase("baium_despawn") && npc != null)
		{
			if (npc.getNpcId() == LIVE_BAIUM)
			{
				...
				else if ((_LastAttackVsBaiumTime + 300000 < System.currentTimeMillis()) && npc.getCurrentHp() < ((npc.getMaxHp() * 3) / 4.0))
				{
					...
					npc.teleToLocation(STONE_LOC[0], STONE_LOC[1], STONE_LOC[2]);
			}
		}
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
				{
					...
					public void run()
					{
						try
						{
							...
							_baium.setRunning();
							_baium.broadcastPacket(new SocialAction(_baium.getObjectId(), 2));
							startQuestTimer("baium_wakeup", 15000, _baium, _player);
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet)
	{
		...
		else if (npc.getNpcId() == ARCHANGEL)
		{
			if ((Rnd.get(100) < 5) && ((npc.getStatus().getCurrentHp() / npc.getStat().getMaxHp())*100) < 50)
			{
				npc.setTarget(npc);
				npc.doCast(SkillTable.getInstance().getInfo(4133, 1));
			}
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		final long respawnTime = (long) Config.Interval_Of_Baium_Spawn + Rnd.get(Config.Random_Of_Baium_Spawn);
	...
	public void doAttack (L2PcInstance player) 
	{
		for(L2Npc npc : _Minions)
		{
			if(npc.getTarget() != null && npc.getTarget() != _baium)
				continue;
	...
	public L2Character getRandomTarget(L2Npc npc)
	{
		...
		final Collection<L2Object> objs = npc.getKnownList().getKnownObjects().values();
		{
			for (final L2Object obj : objs)
			{
				...
				{
					if (((L2Character) obj).getZ() < (npc.getZ() - 100)
