...
public class AIOSchemeHandler implements IAIOItemHandler
{
	...
	public void onBypassUse(L2PcInstance player, String command) 
	{
		...
		{
			if(player.getProfileNames().size() == ExternalConfig.AIOITEM_SCHEME_MAX_PROFILES)
			{
	...
	private boolean paymentDone(L2PcInstance player, Action action)
	{
		...
		if((coin = player.getInventory().getItemByItemId(ExternalConfig.AIOITEM_SCHEME_COIN)) == null)
		{
		...
		switch(action)
		{
			case CREATE_PROFILE:
			{
				if(coin.getCount() < ExternalConfig.AIOITEM_SCHEME_PROFILE_PRICE)
				{
				...
				{
					player.destroyItemByItemId("AIOItem", coin.getItemId(), ExternalConfig.AIOITEM_SCHEME_PROFILE_PRICE, player, true);
					return true;
				}
			}
			case GET_BUFFS:
			{
				if(coin.getCount() < ExternalConfig.AIOITEM_SCHEME_PRICE)
				{
					player.sendMessage("You dont have enought "+coin.getName()+" to create a new scheme!");
					return false;
				}
				...
				{
					player.destroyItemByItemId("AIOItem", coin.getItemId(), ExternalConfig.AIOITEM_SCHEME_PRICE, player, true);
					return true;
				}
			}
			default:
