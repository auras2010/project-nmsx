...
import l2.universe.gameserver.model.actor.instance.L2ControllableMobInstance;
...
public class AdminKill implements IAdminCommandHandler
{
				...
				if (plyr != null)
				{
					if (st.hasMoreTokens())
					{
						try
						{
							...
							final Collection<L2Character> knowChars = plyr.getKnownList().getKnownCharactersInRadius(radius);
							for (final L2Character knownChar : knowChars)
							{
					...
					try
					{
						...
						final Collection<L2Character> knowChars = activeChar.getKnownList().getKnownCharactersInRadius(radius);
						for (final L2Character knownChar : knowChars)
						{
							if (knownChar instanceof L2ControllableMobInstance || knownChar == activeChar
									|| (!(knownChar instanceof L2Attackable) && !(knownChar instanceof L2Playable)))
								continue;
	...
	private void kill(final L2PcInstance activeChar, final L2Character target)
	{
		...
		else if (Config.MOD_CHAMPION_ENABLE && target.isChampion())
			target.reduceCurrentHp(target.getMaxHp() * Config.MOD_CHAMPION_HP + 1, activeChar, null);
		else
		{
			if (target.isInvul())
