...
import l2.universe.gameserver.taskmanager.AutoAnnounceTaskManager.AutoAnnouncement;
...
{	
	...
	public boolean useAdminCommand(String command, final L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_wannounce"))
		{
			...
			{
				if (player != null)
					player.sendPacket(new ExShowScreenMessage(message, Config.WELCOME_MESSAGE_TIME));
			}
		}
		else if (command.startsWith("admin_mannounce"))
		{
			final String message = command.substring(16);
			final Collection<L2PcInstance> pls = L2World.getInstance().getAllPlayers().values();
			for (final L2PcInstance player : pls)
			...
			Announcements.getInstance().handleAnnounce(message, 0);
		}
