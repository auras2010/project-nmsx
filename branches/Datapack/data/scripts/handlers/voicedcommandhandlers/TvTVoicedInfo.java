...
{
	private static final String[] _voicedCommands = { "tvtstatus" };
	...
	{
		if (command.equalsIgnoreCase("tvtstatus"))
		{
			if (TvTEvent.isStarting() || TvTEvent.isStarted())
			{
				...
				try
				{
					...
					npcHtmlMessage.replace("%team1name%", ExternalConfig.TVT_EVENT_TEAM_1_NAME);
					...
					npcHtmlMessage.replace("%team2name%", ExternalConfig.TVT_EVENT_TEAM_2_NAME);
			...
			{
				activeChar.sendMessage("You can't use this command now. Use this command when will be time for TVT Event registration");
