@echo off
REM ####################################################
REM ## l2universe Database Installer                 ##
REM ####################################################
REM ## Interactive script setup                       ##
REM ####################################################
REM Copyright (C) 2010 l2universe
...
REM 675 Mass Ave, Cambridge, MA 02139, USA. Or contact the Official l2universe
REM Project at http://www.l2Universe.info
...
:loadconfig
title l2universe installer - Reading configuration from file...
cls
...
echo and reading about it. Setup and host an l2universe server requires a
...
set /P lsport="Port (default is '%lsport%'): "
...
if NOT "%lsport%"=="%gsport%" set gsport=%lsport%
echo.
cls
echo.
...
set /P cbport="Port (default is '%cbport%'): "
echo.
...
set /P gsport="Port (default is '%gsport%'): "
echo.
...
echo set lsport=%lsport% >> %config_file%
...
echo set cbport=%cbport% >> %config_file%
...
echo set gsport=%gsport%>> %config_file%
...
set cmdline=
set stage=1
title l2universe installer - Login Server database setup
echo.
echo Trying to make a backup of your loginserver database.
set cmdline="%mysqldumpPath%" --add-drop-table -h %lshost% --port %lsport% -u %lsuser% --password=%lspass% %lsdb% ^> "%backup%\loginserver_backup.sql" 2^> NUL
%cmdline%
if %ERRORLEVEL% == 0 goto lsdbok
REM if %safe_mode% == 1 goto omfg
:ls_err1
call :colors 47
title l2universe installer - Login Server database setup ERROR!!!
cls
echo.
...
cls
call :colors 57
title l2universe installer - potential PICNIC detected at stage %stage%
echo.
...
set cmdline=
set stage=2
title l2universe installer - Login Server database setup - DB Creation
echo.
echo Trying to create a Login Server database...
set cmdline="%mysqlPath%" -h %lshost% --port %lsport% -u %lsuser% --password=%lspass% -e "CREATE DATABASE %lsdb%" 2^> NUL
%cmdline%
if %ERRORLEVEL% == 0 goto logininstall
if %safe_mode% == 1 goto omfg
:ls_err2
call :colors 47
title l2universe installer - Login Server database setup - DB Creation error
cls
...
:lsdbok
call :colors 17
title l2universe installer - Login Server database setup - WARNING!!!
echo.
:asklogin
if %fresh_setup%==0 (
set loginprompt=s
...
call :colors 17
set cmdline=
title l2universe installer - Login Server database setup - Full install
echo Deleting loginserver tables for new content.
set cmdline="%mysqlPath%" -h %lshost% --port %lsport% -u %lsuser% --password=%lspass% -D %lsdb% ^< login_install.sql 2^> NUL
%cmdline%
...
set stage=4
title l2universe installer - Community Board Server database setup
echo.
echo Trying to make a backup of your cbserver database.
set cmdline="%mysqldumpPath%" --add-drop-table -h %cbhost% --port %cbport% -u %cbuser% --password=%cbpass% %cbdb% ^> "%backup%\cbserver_backup.sql" 2^> NUL
%cmdline%
if %ERRORLEVEL% == 0 goto cbdbok
REM if %safe_mode% == 1 goto omfg
:cb_err1
call :colors 47
title l2universe installer - Community Board Server database setup ERROR!!!
cls
echo.
echo Backup attempt failed! A possible reason for this to 
echo happen, is that your DB doesn't exist yet. I could 
...
set cmdline=
set stage=5
title l2universe installer - Communty Board Server database setup - DB Creation
echo.
echo Trying to create a Community Board Server database...
set cmdline="%mysqlPath%" -h %cbhost% --port %cbport% -u %cbuser% --password=%cbpass% -e "CREATE DATABASE %cbdb%" 2^> NUL
%cmdline%
if %ERRORLEVEL% == 0 goto communityinstall
if %safe_mode% == 1 goto omfg
:cb_err2
call :colors 47
title l2universe installer - Community Board Server database setup - DB Creation error
cls
echo An error occured while trying to create a database for 
...
:cbdbok
call :colors 17
title l2universe installer - Community Board Server database setup - WARNING!!!
echo.
:askcommunity
if %fresh_setup%==0 (
...
call :colors 17
set cmdline=
title l2universe installer - Community Board Server database setup - Full install
echo Deleting communityserver tables for new content.
set cmdline="%mysqlPath%" -h %cbhost% --port %cbport% -u %cbuser% --password=%cbpass% -D %cbdb% ^< community_install.sql 2^> NUL
%cmdline%
if not %ERRORLEVEL% == 0 goto omfg
goto upgradecbinstall

:upgradecbinstall
set project=l2jserver
set stage=6
set cmdline=
if %full% == 1 (
title l2universe installer - Community Board Server database setup - Installing...
echo Installing new communityserver content.
) else (
title l2universe installer - Community Board Server database setup - Upgrading...
...
set stage=7
title l2universe installer - Game server database setup
cls
echo.
echo Making a backup of the original gameserver database.
set cmdline="%mysqldumpPath%" --add-drop-table -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% %gsdb% ^> "%backup%\gameserver_backup.sql" 2^> NUL
%cmdline%
if %ERRORLEVEL% == 0 goto gsdbok
rem if %safe_mode% == 1 goto omfg
:gs_err1
call :colors 47
title l2universe installer - Game Server database setup - Backup failed!
cls
echo.
...
set stage=8
set cmdline=
title l2universe installer - Game Server database setup - DB Creation
cls
echo Trying to create Game Server database...
set cmdline="%mysqlPath%" -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% -e "CREATE DATABASE %gsdb%" 2^> NUL
%cmdline%
if %ERRORLEVEL% == 0 goto fullinstall
if %safe_mode% == 1 goto omfg
:gs_err2
call :colors 47
title l2universe installer - Game Server database setup - DB Creation failed!
cls
echo.
echo An error occured while trying to create a database for 
...
:gsdbok
call :colors 17
title l2universe installer - Game Server database setup - WARNING!!!
cls
echo.
:asktype
set installtype=u
...
set stage=9
set cmdline=
title l2universe installer - Game Server database setup - Full install
echo Deleting all gameserver tables for new content...
set cmdline="%mysqlPath%" -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% -D %gsdb% ^< full_install.sql 2^> NUL
%cmdline%
if not %ERRORLEVEL% == 0 goto omfg
set full=1
echo.
...
set cmdline=
if %full% == 1 (
title l2universe installer - Game Server database setup - Installing...
echo Installing new gameserver content.
) else (
title l2universe installer - Game Server database setup - Upgrading...
...
:rCMD
echo.
if "%project%"=="l2jserver" set tcommand=..\sql\%dest%_sql
if "%project%"=="l2universe" set tcommand=..\sql\l2universe\%dest%
if "%dest%"=="custom" set tcommand=..\sql\gs_sql\%dest%

for /f %%i in ('DIR %tcommand%\*.sql /B') do call :dump %%i %tcommand%

:checkCMD
if "%dest%"=="ls" set dest=gs&goto rCMD
if "%dest%"=="cb" goto cbupInstCD
if "%dest%"=="needed" set dest=added&goto rCMD

echo done...
echo.
if "%dest%"=="added" goto custom
if "%dest%"=="custom" goto csContinue
goto l2universeNeeded

:l2universeNeeded
set project=l2universe
set cmdline=
set dest=needed

echo Installing needed gameserver procedures, functions...
goto rCMD
...
echo %action% %~nx1
if "%dest%"=="ls" set cmdline="%mysqlPath%" -h %lshost% --port %lsport% -u %lsuser% --password=%lspass% -D %lsdb% ^< %2\%1 2^>^>"%output%" 
if "%dest%"=="cb" set cmdline="%mysqlPath%" -h %cbhost% --port %cbport% -u %cbuser% --password=%cbpass% -D %cbdb% ^< %2\%1 2^>^>"%output%" 
if "%dest%"=="gs" set cmdline="%mysqlPath%" -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% -D %gsdb% ^< %2\%1 2^>^>"%output%" 
if "%project%"=="l2universe" set cmdline="%mysqlPath%" -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% -D %gsdb% ^< %2\%1 2^>^>"%output%"
%cmdline%
...
cls
call :colors 47
title l2universe installer - potential database issue at stage %stage%
echo.
echo Something caused an error while executing instruction :
echo %mysqlPath% -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% -D %gsdb%
echo.
echo with file %~nx1
echo.
...
cls
call :colors 17
title l2universe installer - Game Server database setup - Logging options turned on
set logging=1
if %full% == 1 (
...
pause>NUL
set cmdline="%mysqlPath%" -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% -D %gsdb% ^<..\sql\%1 2^>^>"%output%"
date /t >"%output%"
time /t >>"%output%"
%cmdline%
...
:csContinue
title l2universe installer - Game Server database setup - custom tables processing complete
cls
...
echo.
pause
title l2universe installer - Game Server database setup - L2J Mods
cls
echo l2universe provides a basic infraestructure for some non-retail features
echo (aka l2universe mods) to get enabled with a minimum of changes.
echo.
...
echo.
REM l2universe mods that needed extra tables to work properly, should be 
...
if /i %modprompt%==y "%mysqlPath%" -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% -D %gsdb% < ..\sql\gs_sql\mods\mods_wedding.sql 2>>NUL

title l2universe installer - Game Server database setup - l2universe Mods setup complete
cls
echo Database structure for l2universe mods finished.
echo.
echo Remember that in order to get these additions actually working 
echo you need to edit your configuration files. 
echo.
...
call :colors 17
set stage=10
title l2universe installer - Game/CB Server database setup - update SQL files
cls
if %full% == 1 goto end
echo.
...
for %%i in (*.sql) do echo "%mysqlPath%" -h %gshost% --port %gsport% -u %gsuser% --password=%gspass% -D %gsdb% ^< %%i 2^>^> errors.txt >> temp.bat
call temp.bat> nul
del temp.bat
...
for %%i in (*.sql) do echo "%mysqlPath%" -h %cbhost% --port %gsport% -u %cbuser% --password=%cbpass% -D %cbdb% ^< %%i 2^>^> cberrors.txt >> temp.bat
call temp.bat> nul
del temp.bat
move cberrors.txt %workdir%
cd %workdir%
:nbfinished
title l2universe installer - Game Server database setup - updates processing complete
cls
...
:horrible_end
call :colors 47
title l2universe installer - Oops!
cls
echo This wasn't a clean run, but don't worry.
echo You can get help and support:
echo.
echo 1-Search for a similar problem in our forums :
echo       (http://www.l2Universe.info)
echo.
...
set dpvf="..\config\l2universedp-version.properties" 
...
:end
call :colors 17
title l2universe installer - Script execution finished
cls
echo.
echo l2universe database_installer version 0.2.2
echo (C) 2007-2011 l2universe Team
...
echo visit http://www.l2universe.info for more info about
echo the Official l2universe project.
echo.
pause
color
